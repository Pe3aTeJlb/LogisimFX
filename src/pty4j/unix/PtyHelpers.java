...
import pty4j.util.LazyValue;
import pty4j.util.PtyUtil;
 ...
 */
  ...
  private static final LazyValue<OSFacade> OS_FACADE_VALUE = new LazyValue<>(() -> {
    if (Platform.isMac()) {
      return new pty4j.unix.macosx.OSFacadeImpl();
    }
    if (Platform.isFreeBSD()) {
      return new pty4j.unix.freebsd.OSFacadeImpl();
    }
    if (Platform.isOpenBSD()) {
      return new pty4j.unix.openbsd.OSFacadeImpl();
    }
    if (Platform.isLinux() || Platform.isAndroid()) {
      return new pty4j.unix.linux.OSFacadeImpl();
    }
