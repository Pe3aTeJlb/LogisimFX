...
if (!String.prototype.padStart) {
    String.prototype.padStart = function(targetLength, padString) { // If the string is already long enough, nothing to do!
        ...
        if (padString === undefined) padString = ' '; // In case the pad is multiple chars long.
...
if (typeof Promise.prototype.finally !== 'function') {
    var speciesConstructor = function speciesConstructor(O, defaultConstructor) {
        ...
        if (!C || _typeof(C) !== 'object' && typeof C !== 'function') {
    ...
    var shim = {
            ...
            var promise = this;
            if (_typeof(promise) !== 'object' || promise === null) {
...
lib.codec.stringToCodeUnitArray = function(str) {
    var type = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : Array; // Indexing string directly is faster than Array.map.
    var ret = new type(str.length);
...
lib.colors.x11HexToCSS = function(v) {
    ...
    var b = v.substr(size + size, size); // Normalize to 16 bits.
    function norm16(v) {
        v = parseInt(v, 16);
...
lib.colors.x11ToCSS = function(v) {
    ...
    if (!ary) { // Handle the legacy format.
        if (v.startsWith('#')) return lib.colors.x11HexToCSS(v);
        else return lib.colors.nameToRGB(v);
    }
    ary.splice(0, 1);
 ...
 */
lib.f.getStack = function() {
    var ignoreFrames = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
    var count = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : undefined;
    ...
    else count = lib.f.clamp(count, 0, max); // Remove the leading spaces and "at" from each line:
    // '    at window.onload (file:///.../lib_test.js:11:18)'
    var stackObject = new Array();
    for (var i = ignoreFrames; i < count + ignoreFrames; ++i) {
...
lib.f.getOs = function() { // Try the brower extensions API.
    ...
    if (window.navigator && navigator.userAgent) {
        ...
        if (ua.includes('Mac OS X')) return Promise.resolve('mac');
        else if (ua.includes('CrOS')) return Promise.resolve('cros');
        else if (ua.includes('Linux')) return Promise.resolve('linux');
        else if (ua.includes('Android')) return Promise.resolve('android');
        else if (ua.includes('Windows')) return Promise.resolve('windows');
    } // Still here?  No idea.
...
lib.f.lastError = function() {
    ...
    var lastError;
    if (window.browser && browser.runtime) lastError = browser.runtime.lastError;
    else if (window.chrome && chrome.runtime) lastError = chrome.runtime.lastError;
    if (lastError && lastError.message) return lastError.message;
 ...
 */
lib.f.openWindow = function(url) {
    var name = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : undefined;
    var features = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined; // We create the window first without the URL loaded.
 ...
 */
lib.i18n.getMessage = function(msgname) {
    var substitutions = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
    var fallback = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : ''; // First let the native browser APIs handle everything for us.
    if (lib.i18n.browser_) {
 ...
 */
lib.i18n.replaceReferences = function(msg) {
    var args = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : []; // The Chrome API allows a single substitution as a string rather than array.
    if (args === null) {
        args = [];
    }
    if (!_instanceof(args, Array)) {
        args = [args];
    }
...
lib.MessageManager.prototype.addMessages = function(defs) {
    for (var key in defs) {
        ...
        } else { // Replace "$NAME$" placeholders with "$1", etc.
...
lib.MessageManager.prototype.get = function(msgname, opt_args, opt_default) { // First try the integrated browser getMessage.  We prefer that over any
    ...
    if (message) return message; // Look it up in the registered cache next.
    message = this.messages[msgname];
    if (!message) {
        console.warn('Unknown message: ' + msgname);
        message = opt_default === undefined ? msgname : opt_default; // Register the message with the default to avoid multiple warnings.
...
lib.MessageManager.prototype.processI18nAttribute = function(node) { // Convert the "lower-and-dashes" attribute names into
    ...
    for (var key in i18n) { // The node attribute we'll be setting.
        var attr = key;
        var msgname = i18n[key]; // For "=foo", re-use the referenced message name.
        ...
        if (attr == '_') node.textContent = msg;
        else node.setAttribute(attr, msg);
    }
...
lib.PreferenceManager = function(storage, opt_prefix) {
    ...
    this.prefix = prefix; // Internal state for when we're doing a bulk import from JSON and we want
    // to elide redundant storage writes (for quota reasons).
    this.isImportingJson_ = false;
    this.prefRecords_ = {};
    this.globalObservers_ = [];
    this.childFactories_ = {}; // Map of list-name to {map of child pref managers}
    // As in...
    //
    //  this.childLists_ = {
    //    'profile-ids': {
...
lib.PreferenceManager.Record.prototype.get = function() {
    if (this.currentValue === this.DEFAULT_VALUE) {
        ...
        if (_typeof(this.defaultValue) == 'object') { // We want to return a COPY of the default value so that users can
...
lib.PreferenceManager.prototype.createChild = function(listName, opt_hint, opt_id) {
    ...
    } else { // Pick a random, unique 4-digit hex identifier for the new profile.
...
lib.PreferenceManager.prototype.changeDefault = function(name, newValue) {
    var record = this.prefRecords_[name];
    if (!record) throw new Error('Unknown preference: ' + name);
    if (!this.diff(record.defaultValue, newValue)) { // Default value hasn't changed.
        return;
    }
...
lib.PreferenceManager.prototype.set = function(name, newValue) {
    var onComplete = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
    var saveToStorage = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
    var record = this.prefRecords_[name];
    if (!record) throw new Error('Unknown preference: ' + name);
    ...
    if (this.diff(record.defaultValue, newValue)) {
        ...
        if (saveToStorage) this.storage.setItem(this.prefix + name, newValue, onComplete);
    } else {
        record.currentValue = this.DEFAULT_VALUE;
        if (saveToStorage) this.storage.removeItem(this.prefix + name, onComplete);
...
lib.PreferenceManager.prototype.importFromJson = function(json, opt_onComplete) {
    ...
    var onWriteStorage = function onWriteStorage() {
        if (--pendingWrites < 1) {
            if (opt_onComplete) opt_onComplete(); // We've delayed updates to the child arrays, so flush them now.
            for (var _name in json) {
                if (_name in _this3.childLists_) _this3.set(_name, _this3.get(_name));
            }
            _this3.isImportingJson_ = false;
        }
    };
...
lib.PreferenceManager.prototype.onStorageChange_ = function(map) {
    for (var key in map) {
        ...
        if (!(name in this.prefRecords_)) { // Sometimes we'll get notified about prefs that are no longer defined.
            continue;
        }
        var record = this.prefRecords_[name];
...
lib.Storage.Chrome.prototype.setItem = function(key, value, opt_callback) {
    ...
    var onComplete = function onComplete() {
        ...
        if (err) { // Doesn't seem to be any better way of handling this.
            // https://crbug.com/764759
            if (err.indexOf('MAX_WRITE_OPERATIONS')) {
                console.warn("Will retry save of ".concat(key, " after exceeding quota: ").concat(err));
                setTimeout(function() {
                    return _this4.setItem(key, value, onComplete);
                }, 1000);
                return;
            } else {
                console.error("Unknown runtime error: ".concat(err));
            }
        }
...
lib.Storage.Local.prototype.getItem = function(key, callback) {
    ...
    if (typeof value == 'string') {
        ...
        } catch (e) { // If we can't parse the value, just return it unparsed.
        }
    }
...
(function(scope) {
    ...
    function FastTextEncoder() {
        var utfLabel = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'utf-8';
        if (utfLabel !== 'utf-8') {
    ...
    FastTextDecoder.prototype.decode = function(buffer) {
        ...
        while (pos < len) {
            ...
            if ((byte1 & 0x80) === 0) { // 1-byte
                ...
                var _byte = bytes[pos++] & 0x3f;
                var byte3 = bytes[pos++] & 0x3f;
                out.push((byte1 & 0x1f) << 12 | _byte << 6 | byte3);
            } else if ((byte1 & 0xf8) === 0xf0) {
                var _byte2 = bytes[pos++] & 0x3f;
                var _byte3 = bytes[pos++] & 0x3f;
                var byte4 = bytes[pos++] & 0x3f; // this can be > 0xffff, so possibly generate surrogates
                var codepoint = (byte1 & 0x07) << 0x12 | _byte2 << 0x0c | _byte3 << 0x06 | byte4;
...
})(typeof window !== 'undefined' ? window : typeof global !== 'undefined' ? global : void 0); // SOURCE FILE: libdot/third_party/wcwidth/lib_wc.js
...
lib.wc.charWidthDisregardAmbiguous = function(ucs) { // Optimize for ASCII characters.
    if (ucs < 0x7f) {
        if (ucs >= 0x20) return 1;
        else if (ucs == 0) return lib.wc.nulWidth;
        else /* if (ucs < 0x20) */ return lib.wc.controlWidth;
...
lib.wc.substr = function(str, start, opt_width) {
    var startIndex = 0;
    var endIndex, width; // Fun edge case: Normally we associate zero width codepoints (like combining
...
lib.resource.add('libdot/changelog/version', 'text/plain', '1.26');
lib.resource.add('libdot/changelog/date', 'text/plain', '2019-01-19'); // This file was generated by libdot/bin/concat.sh.
// It has been marked read-only for your safety.  Rather than
// edit it directly, please modify one of these source files.
//
...
lib.registerInit('hterm', function(onInit) {
    ...
    function onTab(tab) {
        ...
        } else { // TODO(rginda): This is where we end up for a v1 app's background page.
    ...
    if (isPackagedApp) { // Packaged apps are never displayed in browser tabs.
...
hterm.copySelectionToClipboard = function(document, str) { // Request permission if need be.
    var requestPermission = function requestPermission() { // Use the Permissions API if available.
        if (navigator.permissions && navigator.permissions.query) {
            return navigator.permissions.query({
                    ...
                    var checkState = function checkState(resolve, reject) {
                        switch (status.state) {
                            case 'granted':
                                return resolve();
                            case 'denied':
                                return reject();
                            default: // Wait for the user to approve/disprove.
                                return new Promise(function(resolve, reject) {
        ...
        } else { // No permissions API, so resolve right away.
            return Promise.resolve();
        }
    ...
    var writeClipboard = function writeClipboard() { // Use the Clipboard API if available.
        if (navigator.clipboard && navigator.clipboard.writeText) { // If this fails (perhaps due to focus changing windows), fallback to the
            // legacy copy method.
            return navigator.clipboard.writeText(str).catch(execCommand);
        } else { // No Clipboard API, so use the old execCommand style.
            return execCommand();
        }
    ...
    var execCommand = function execCommand() {
        ...
        var focusOffset = selection.focusOffset; // FF sometimes throws NS_ERROR_FAILURE exceptions when we make this call.
        ...
        } catch (firefoxException) {} // Ignore this. FF throws an exception if there was an error, even
        ...
        copySource.parentNode.removeChild(copySource); // Since execCommand is synchronous, resolve right away.
        return Promise.resolve();
    }; // Kick it all off!
...
hterm.pasteFromClipboard = function(document) {
    ...
    } catch (firefoxException) { // Ignore this.  FF 40 and older would incorrectly throw an exception if
...
hterm.notify = function(params) {
    var def = function def(curr, fallback) {
        return curr !== undefined ? curr : fallback;
    };
    if (params === undefined || params === null) params = {}; // Merge the user's choices with the default settings.  We don't take it
    ...
    if (!title) title = 'hterm';
    title = lib.f.replaceVars(hterm.desktopNotificationTitle, {
        'title': title
    });
...
hterm.openUrl = function(url) {
    if (window.chrome && chrome.browser && chrome.browser.openTab) { // For Chrome v2 apps, we need to use this API to properly open windows.
        ...
        var win = lib.f.openWindow(url, '_blank');
        win.focus();
    }
};
/**
...
hterm.AccessibilityReader = function(div) {
    ...
    div.appendChild(liveRegion); // Whether command output should be rendered for Assistive Technology.
    ...
    liveRegion.appendChild(this.liveElement_); // This live element is used for speaking out the current screen when
    ...
    liveRegion.appendChild(this.assertiveLiveElement_); // A queue of updates to announce.
...
hterm.AccessibilityReader.prototype.decorate = function(doc) {
    var _this5 = this;
    var handlers = ['keydown', 'keypress', 'keyup', 'textInput'];
    handlers.forEach(function(handler) {
        doc.addEventListener(handler, function() {
            _this5.hasUserGesture = true;
        });
    });
};
/**
...
hterm.AccessibilityReader.prototype.afterCursorChange = function(cursorRowString, cursorRow, cursorColumn) { // This can happen if clear() is called midway through a cursor change.
    ...
    if (!this.announceAction_(cursorRowString, cursorRow, cursorColumn)) { // If we don't announce a special action, we re-queue all the output that
...
hterm.AccessibilityReader.prototype.announce = function(str) {
    ...
    if (str == '\n' && this.queue_.length > 0) {
        this.queue_.push(''); // We don't need to trigger an announcement on newlines because they won't
        // change the existing content that's output.
        return;
    }
    if (this.queue_.length == 0) {
        this.queue_.push(str);
    } else { // We put a space between strings that appear on the same line.
    ...
    if (this.queue_.length == 1) {
        ...
        throw new Error('Expected only one item in queue_ or nextReadTimer_ to be running.');
    }
};
/**
...
hterm.AccessibilityReader.prototype.announceAction_ = function(cursorRowString, cursorRow, cursorColumn) { // If the cursor changes rows, we don't announce anything at present.
    ...
    if (this.lastCursorRowString_ == cursorRowString) { // Moving the cursor along the line. We check that no significant changes
        ...
        if (this.lastCursorColumn_ != cursorColumn && this.cursorChangeQueue_.join('').trim() == '') { // Announce the text between the old cursor position and the new one.
            var start = Math.min(this.lastCursorColumn_, cursorColumn);
            var len = Math.abs(cursorColumn - this.lastCursorColumn_);
            this.assertiveAnnounce(lib.wc.substr(this.lastCursorRowString_, start, len));
            return true;
        }
        return false;
    ...
    if (this.lastCursorRowString_ != cursorRowString) { // Spacebar. We manually announce this character since the screen reader may
        ...
        if (this.lastCursorColumn_ + 1 == cursorColumn) {
            if (lib.wc.substr(cursorRowString, cursorColumn - 1, 1) == ' ' && this.cursorChangeQueue_.length > 0 && this.cursorChangeQueue_[0] == ' ') {
        ...
        if (lib.wc.strWidth(cursorRowString) <= lib.wc.strWidth(this.lastCursorRowString_) && lib.wc.substr(this.lastCursorRowString_, 0, cursorDeleted) == lib.wc.substr(cursorRowString, 0, cursorDeleted)) { // Find the length of the current row string ignoring space characters.
            ...
            var lengthOfCurrentRow = lib.wc.strWidth(cursorRowString);
            for (; lengthOfCurrentRow > 0; --lengthOfCurrentRow) {
                if (lengthOfCurrentRow == cursorDeleted || lib.wc.substr(cursorRowString, lengthOfCurrentRow - 1, 1) != ' ') {
                    break;
                }
            }
            var numCharsDeleted = lib.wc.strWidth(this.lastCursorRowString_) - lengthOfCurrentRow; // Check that the end of the strings match.
            var lengthOfEndOfString = lengthOfCurrentRow - cursorDeleted;
            var endOfLastRowString = lib.wc.substr(this.lastCursorRowString_, cursorDeleted + numCharsDeleted, lengthOfEndOfString);
            var endOfCurrentRowString = lib.wc.substr(cursorRowString, cursorDeleted, lengthOfEndOfString);
            if (endOfLastRowString == endOfCurrentRowString) {
                var deleted = lib.wc.substr(this.lastCursorRowString_, cursorDeleted, numCharsDeleted);
                if (deleted != '') {
...
hterm.ContextMenu.prototype.regenerate_ = function() {
    ...
    this.menu_.forEach(function(_ref) {
        var _ref2 = _slicedToArray(_ref, 2),
            name = _ref2[0],
            action = _ref2[1];
        var menuitem = _this6.document_.createElement('menuitem');
        ...
        _this6.element_.appendChild(menuitem);
    });
};
/**
...
hterm.ContextMenu.prototype.show = function(e, terminal) { // If there are no menu entries, then don't try to show anything.
    ...
    this.element_.style.top = "".concat(e.clientY, "px");
    this.element_.style.left = "".concat(e.clientX, "px");
    var docSize = hterm.getClientSize(this.document_.body);
    this.element_.style.display = 'block'; // We can't calculate sizes until after it's displayed.
    var eleSize = hterm.getClientSize(this.element_); // Make sure the menu isn't clipped outside of the current element.
    var minY = Math.max(0, docSize.height - eleSize.height);
    var minX = Math.max(0, docSize.width - eleSize.width);
    if (minY < e.clientY) {
        this.element_.style.top = "".concat(minY, "px");
    }
    if (minX < e.clientX) {
        this.element_.style.left = "".concat(minX, "px");
    }
};
/**
...
hterm.Frame.prototype.onMessage_ = function(e) {
    switch (e.data.name) {
        case 'ipc-init-ok': // We get this response after we send them ipc-init and they finish.
            ...
            return;
        case 'terminal-info-ok': // We get this response after we send them terminal-info and they finish.
...
hterm.Frame.prototype.show = function() {
    ...
    if (false) { // No use for the close button, so no use for the window header either.
        var header = document.createElement('div');
        header.style.cssText = 'display: flex;' + 'justify-content: flex-end;' + 'height: ' + headerHeight + ';' + 'background-color: ' + this.terminal_.getForegroundColor() + ';' + 'color: ' + this.terminal_.getBackgroundColor() + ';' + 'font-size: 16px;' + 'font-family: ' + this.terminal_.getFontFamily();
...
hterm.Keyboard.prototype.onKeyPress_ = function(e) { // FF doesn't set keyCode reliably in keypress events.  Stick to the which
    ...
    if ((e.ctrlKey || e.metaKey) && (key == 'c' || key == 'v')) { // On FF the key press (not key down) event gets fired for copy/paste.
        // Let it fall through for the default browser behavior.
        return;
    }
    if (e.keyCode == 9 /* Tab */ ) { // On FF, a key press event will be fired in addition of key down for the
...
hterm.Keyboard.prototype.onKeyDown_ = function(e) {
    ...
    if (!keyDef) { // If this key hasn't been explicitly registered, fall back to the unknown
        // key mapping (keyCode == 0), and then automatically register it to avoid
        // any further warnings here.
        console.warn("No definition for key ".concat(e.key, " (keyCode ").concat(e.keyCode, ")"));
    ...
    function getAction(name) { // Get the key action for the given action name.  If the action is a
    ...
    var meta = this.altIsMeta ? e.altKey || e.metaKey : e.metaKey; // In the key-map, we surround the keyCap for non-printables in "[...]"
    var isPrintable = !/^\[\w+\]$/.test(keyDef.keyCap);
    switch (this.altGrMode) {
        case 'ctrl-alt':
            if (isPrintable && control && alt) { // ctrl-alt-printable means altGr.  We clear out the control and
    ...
    if (binding) { // Clear out the modifier bits so we don't try to munge the sequence
        // further.
    ...
    if (typeof action == 'string' && action.substr(0, 2) == '\x1b[' && (alt || control || shift || meta)) { // The action is an escape sequence that and it was triggered in the
        ...
        if (meta) imod += 8;
        var mod = ';' + imod;
        if (action.length == 3) { // Some of the CSI sequences have zero parameters unless modified.
        ...
        if (alt && this.altSendsWhat == 'escape' || meta && this.metaSendsEscape) {
            action = '\x1b' + action;
        }
    }
...
hterm.Keyboard.KeyMap.prototype.reset = function() {
    this.keyDefs = {};
    var self = this; // This function is used by the "macro" functions below.  It makes it
    ...
    if (window.navigator && navigator.userAgent) {
        if (navigator.userAgent.includes('Firefox')) { // Firefox defines some keys uniquely.  No other browser defines these is
    ...
    var PASS = hterm.Keyboard.KeyActions.PASS;
    var STRIP = hterm.Keyboard.KeyActions.STRIP;
    this.addKeyDefs( // These fields are: [keycode, keycap, normal, control, alt, meta]
...
hterm.Keyboard.KeyMap.prototype.onKeyDel_ = function(e) {
    if (this.keyboard.altBackspaceIsMetaBackspace && this.keyboard.altKeyPressed && !e.altKey) return '\x1b\x7f';
    return '\x1b[3~';
};
/**
 * Either scroll the scrollback buffer or send a key sequence.
...
hterm.Keyboard.KeyMap.prototype.onKeyArrowUp_ = function(e) {
    ...
    return e.shiftKey || e.ctrlKey || e.altKey || e.metaKey || !this.keyboard.applicationCursor ? '\x1b[A' : '\x1bOA';
};
/**
 * Either scroll the scrollback buffer or send a key sequence.
 */
hterm.Keyboard.KeyMap.prototype.onKeyArrowDown_ = function(e) {
    ...
    return e.shiftKey || e.ctrlKey || e.altKey || e.metaKey || !this.keyboard.applicationCursor ? '\x1b[B' : '\x1bOB';
};
/**
...
hterm.Keyboard.KeyMap.prototype.onF11_ = function(e, keyDef) {
    if (hterm.windowType != 'popup') return hterm.Keyboard.KeyActions.PASS;
    else return '\x1b[23~';
};
/**
...
hterm.Keyboard.KeyMap.prototype.onCtrlN_ = function(e, keyDef) {
    if (e.shiftKey) {
        lib.f.openWindow(document.location.href, '', 'chrome=no,close=yes,resize=yes,scrollbars=yes,' + 'minimizable=yes,width=' + window.innerWidth + ',height=' + window.innerHeight);
        return hterm.Keyboard.KeyActions.CANCEL;
    }
    return '\x0e';
};
...
hterm.Keyboard.KeyMap.prototype.onCtrlV_ = function(e, keyDef) {
    if (!e.shiftKey && this.keyboard.ctrlVPaste || e.shiftKey && !this.keyboard.ctrlVPaste) { // We try to do the pasting ourselves as not all browsers/OSs bind Ctrl-V to
        ...
        if (this.keyboard.terminal.paste()) return hterm.Keyboard.KeyActions.CANCEL;
        else return hterm.Keyboard.KeyActions.PASS;
    }
    return '\x16';
};
/**
...
hterm.Keyboard.KeyMap.prototype.onMetaN_ = function(e, keyDef) {
    if (e.shiftKey) {
        lib.f.openWindow(document.location.href, '', 'chrome=no,close=yes,resize=yes,scrollbars=yes,' + 'minimizable=yes,width=' + window.outerWidth + ',height=' + window.outerHeight);
        return hterm.Keyboard.KeyActions.CANCEL;
    }
    return hterm.Keyboard.KeyActions.DEFAULT;
};
...
hterm.Keyboard.KeyMap.prototype.onMetaC_ = function(e, keyDef) {
    ...
    if (e.shiftKey || document.getSelection().isCollapsed) { // If the shift key is being held, or there is no document selection, send
    ...
    while (this.pos < this.source.length) {
        ...
        if (token.type == 'integer') {
            ...
            if (ucValue in hterm.Parser.identifiers.modifierKeys && hterm.Parser.identifiers.modifierKeys.hasOwnProperty(ucValue)) {
                ...
                rv[mod] = true;
            } else if (ucValue in hterm.Parser.identifiers.keyCodes && hterm.Parser.identifiers.keyCodes.hasOwnProperty(ucValue)) {
...
hterm.Parser.prototype.parseKeyAction = function() {
    ...
    if (token.type == 'identifier') {
        if (token.value in hterm.Parser.identifiers.actions && hterm.Parser.identifiers.actions.hasOwnProperty(token.value)) return hterm.Parser.identifiers.actions[token.value];
...
hterm.Parser.prototype.parseInteger = function() {
    var base = 10;
    if (this.ch == '0' && this.pos < this.source.length - 1 && this.source.substr(this.pos + 1, 1) == 'x') {
...
hterm.Parser.prototype.parseEscape = function() {
    ...
    if (!(this.ch in map && map.hasOwnProperty(this.ch))) throw this.error('Unknown escape: ' + this.ch);
...
hterm.Parser.identifiers.keyCodes = { // Top row.
    ...
    J: 74,
    K: 75,
    L: 76, // We map enter and return together even though enter should really be 10
    ...
    KP_DIVIDE: 111,
    KP_DECIMAL: 110,
    KP_PERIOD: 110, // Chrome OS media keys
    NAVIGATE_BACK: 166,
...
hterm.Parser.identifiers.actions = {
     ...
     */
    scrollLineUp: function scrollLineUp(terminal) {
     ...
     */
    scrollLineDown: function scrollLineDown(terminal) {
     ...
     */
    scrollPageUp: function scrollPageUp(terminal) {
     ...
     */
    scrollPageDown: function scrollPageDown(terminal) {
     ...
     */
    scrollToTop: function scrollToTop(terminal) {
        terminal.scrollHome();
        return hterm.Keyboard.KeyActions.CANCEL;
    },
    /**
     ...
     */
    scrollToBottom: function scrollToBottom(terminal) {
        terminal.scrollEnd();
        return hterm.Keyboard.KeyActions.CANCEL;
    },
    /**
     ...
     */
    clearScreen: function clearScreen(terminal) {
        terminal.clearHome();
        return hterm.Keyboard.KeyActions.CANCEL;
    },
    /**
     ...
     */
    clearScrollback: function clearScrollback(terminal) {
     ...
     */
    clearTerminal: function clearTerminal(terminal) {
     ...
     */
    fullReset: function fullReset(terminal) {
        terminal.reset();
        return hterm.Keyboard.KeyActions.CANCEL;
    },
    /**
     ...
     */
    softReset: function softReset(terminal) {
        terminal.softReset();
        return hterm.Keyboard.KeyActions.CANCEL;
    }
...
hterm.PreferenceManager = function(profileId) {
    ...
    Object.entries(hterm.PreferenceManager.defaultPreferences).forEach(function(_ref3) {
        var _ref4 = _slicedToArray(_ref3, 2),
            key = _ref4[0],
            entry = _ref4[1];
        _this7.definePreference(key, entry['default']);
    });
};
/**
...
hterm.PreferenceManager.listProfiles = function(callback) {
    hterm.defaultStorage.getItems(null, function(items) {
        var profiles = {};
        for (var _i3 = 0, _Object$keys = Object.keys(items); _i3 < _Object$keys.length; _i3++) {
            var key = _Object$keys[_i3];
            if (key.startsWith(hterm.PreferenceManager.prefix_)) { // Turn "/hterm/profiles/foo/bar/cow" to "foo/bar/cow".
...
hterm.PreferenceManager.defaultPreferences = {
    'alt-gr-mode': hterm.PreferenceManager.definePref_('AltGr key mode', hterm.PreferenceManager.categories.Keyboard, null, [null, 'none', 'ctrl-alt', 'left-alt', 'right-alt'], "Select an AltGr detection heuristic.\n" + "\n" + "'null': Autodetect based on navigator.language:\n" + "      'en-us' => 'none', else => 'right-alt'\n" + "'none': Disable any AltGr related munging.\n" + "'ctrl-alt': Assume Ctrl+Alt means AltGr.\n" + "'left-alt': Assume left Alt means AltGr.\n" + "'right-alt': Assume right Alt means AltGr."),
    'alt-backspace-is-meta-backspace': hterm.PreferenceManager.definePref_('Alt-Backspace is Meta-Backspace', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "If set, undoes the Chrome OS Alt-Backspace->DEL remap, so that " + "Alt-Backspace indeed is Alt-Backspace."),
    'alt-is-meta': hterm.PreferenceManager.definePref_('Treat Alt key as Meta key', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "Whether the Alt key acts as a Meta key or as a distinct Alt key."),
    'alt-sends-what': hterm.PreferenceManager.definePref_('Alt key modifier handling', hterm.PreferenceManager.categories.Keyboard, 'escape', ['escape', '8-bit', 'browser-key'], "Controls how the Alt key is handled.\n" + "\n" + "  escape: Send an ESC prefix.\n" + "  8-bit: Add 128 to the typed character as in xterm.\n" + "  browser-key: Wait for the keypress event and see what the browser\n" + "    says. (This won't work well on platforms where the browser\n" + "    performs a default action for some Alt sequences.)"),
    'audible-bell-sound': hterm.PreferenceManager.definePref_('Alert bell sound (URI)', hterm.PreferenceManager.categories.Sounds, 'lib-resource:hterm/audio/bell', 'url', "URL of the terminal bell sound. Empty string for no audible bell."),
    'desktop-notification-bell': hterm.PreferenceManager.definePref_('Create desktop notifications for alert bells', hterm.PreferenceManager.categories.Sounds, false, 'bool', "If true, terminal bells in the background will create a Web " + "Notification. https://www.w3.org/TR/notifications/\n" + "\n" + "Displaying notifications requires permission from the user. When this " + "option is set to true, hterm will attempt to ask the user for " + "permission if necessary. Browsers may not show this permission " + "request if it was not triggered by a user action.\n" + "\n" + "Chrome extensions with the \"notifications\" permission have permission " + "to display notifications."),
    'background-color': hterm.PreferenceManager.definePref_('Background color', hterm.PreferenceManager.categories.Appearance, 'rgb(16, 16, 16)', 'color', "The background color for text with no other color attributes."),
    'background-image': hterm.PreferenceManager.definePref_('Background image', hterm.PreferenceManager.categories.Appearance, '', 'string', "CSS value of the background image. Empty string for no image.\n" + "\n" + "For example:\n" + "  url(https://goo.gl/anedTK)\n" + "  linear-gradient(top bottom, blue, red)"),
    'background-size': hterm.PreferenceManager.definePref_('Background image size', hterm.PreferenceManager.categories.Appearance, '', 'string', "CSS value of the background image size."),
    'background-position': hterm.PreferenceManager.definePref_('Background image position', hterm.PreferenceManager.categories.Appearance, '', 'string', "CSS value of the background image position.\n" + "\n" + "For example:\n" + "  10% 10%\n" + "  center"),
    'backspace-sends-backspace': hterm.PreferenceManager.definePref_('Backspace key behavior', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "If true, the backspace should send BS ('\\x08', aka ^H). Otherwise " + "the backspace key should send '\\x7f'."),
    'character-map-overrides': hterm.PreferenceManager.definePref_('Character map overrides', hterm.PreferenceManager.categories.Appearance, null, 'value', "This is specified as an object. It is a sparse array, where each " + "property is the character set code and the value is an object that is " + "a sparse array itself. In that sparse array, each property is the " + "received character and the value is the displayed character.\n" + "\n" + "For example:\n" + "  {\"0\":{\"+\":\"\\u2192\",\",\":\"\\u2190\",\"-\":\"\\u2191\",\".\":\"\\u2193\", " + "\"0\":\"\\u2588\"}}"),
    'close-on-exit': hterm.PreferenceManager.definePref_('Close window on exit', hterm.PreferenceManager.categories.Miscellaneous, true, 'bool', "Whether to close the window when the command finishes executing."),
    'cursor-blink': hterm.PreferenceManager.definePref_('Cursor blink', hterm.PreferenceManager.categories.Appearance, false, 'bool', "Whether the text cursor blinks by default. This can be toggled at " + "runtime via terminal escape sequences."),
    'cursor-blink-cycle': hterm.PreferenceManager.definePref_('Cursor blink rate', hterm.PreferenceManager.categories.Appearance, [1000, 500], 'value', "The text cursor blink rate in milliseconds.\n" + "\n" + "A two element array, the first of which is how long the text cursor " + "should be on, second is how long it should be off."),
    'cursor-color': hterm.PreferenceManager.definePref_('Text cursor color', hterm.PreferenceManager.categories.Appearance, 'rgba(255, 0, 0, 0.5)', 'color', "The color of the visible text cursor."),
    'color-palette-overrides': hterm.PreferenceManager.definePref_('Initial color palette', hterm.PreferenceManager.categories.Appearance, null, 'value', "Override colors in the default palette.\n" + "\n" + "This can be specified as an array or an object. If specified as an " + "object it is assumed to be a sparse array, where each property " + "is a numeric index into the color palette.\n" + "\n" + "Values can be specified as almost any CSS color value. This " + "includes #RGB, #RRGGBB, rgb(...), rgba(...), and any color names " + "that are also part of the standard X11 rgb.txt file.\n" + "\n" + "You can use 'null' to specify that the default value should be not " + "be changed. This is useful for skipping a small number of indices " + "when the value is specified as an array.\n" + "\n" + "For example, these both set color index 1 to blue:\n" + "  {1: \"#0000ff\"}\n" + "  [null, \"#0000ff\"]"),
    'copy-on-select': hterm.PreferenceManager.definePref_('Automatically copy selected content', hterm.PreferenceManager.categories.CopyPaste, true, 'bool', "Automatically copy mouse selection to the clipboard."),
    'use-default-window-copy': hterm.PreferenceManager.definePref_('Let the browser handle text copying', hterm.PreferenceManager.categories.CopyPaste, false, 'bool', "Whether to use the default browser/OS's copy behavior.\n" + "\n" + "Allow the browser/OS to handle the copy event directly which might " + "improve compatibility with some systems (where copying doesn't work " + "at all), but makes the text selection less robust.\n" + "\n" + "For example, long lines that were automatically line wrapped will " + "be copied with the newlines still in them."),
    'clear-selection-after-copy': hterm.PreferenceManager.definePref_('Automatically clear text selection', hterm.PreferenceManager.categories.CopyPaste, true, 'bool', "Whether to clear the selection after copying."),
    'ctrl-plus-minus-zero-zoom': hterm.PreferenceManager.definePref_('Ctrl-+/-/0 zoom behavior', hterm.PreferenceManager.categories.Keyboard, true, 'bool', "If true, Ctrl-Plus/Minus/Zero controls zoom.\n" + "If false, Ctrl-Shift-Plus/Minus/Zero controls zoom, Ctrl-Minus sends " + "^_, Ctrl-Plus/Zero do nothing."),
    'ctrl-c-copy': hterm.PreferenceManager.definePref_('Ctrl-C copy behavior', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "Ctrl-C copies if true, send ^C to host if false.\n" + "Ctrl-Shift-C sends ^C to host if true, copies if false."),
    'ctrl-v-paste': hterm.PreferenceManager.definePref_('Ctrl-V paste behavior', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "Ctrl-V pastes if true, send ^V to host if false.\n" + "Ctrl-Shift-V sends ^V to host if true, pastes if false."),
    'east-asian-ambiguous-as-two-column': hterm.PreferenceManager.definePref_('East Asian Ambiguous use two columns', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "Whether East Asian Ambiguous characters have two column width."),
    'enable-8-bit-control': hterm.PreferenceManager.definePref_('Support non-UTF-8 C1 control characters', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "True to enable 8-bit control characters, false to ignore them.\n" + "\n" + "We'll respect the two-byte versions of these control characters " + "regardless of this setting."),
    'enable-bold': hterm.PreferenceManager.definePref_('Bold text behavior', hterm.PreferenceManager.categories.Appearance, null, 'tristate', "If true, use bold weight font for text with the bold/bright " + "attribute. False to use the normal weight font. Null to autodetect."),
    'enable-bold-as-bright': hterm.PreferenceManager.definePref_('Use bright colors with bold text', hterm.PreferenceManager.categories.Appearance, true, 'bool', "If true, use bright colors (8-15 on a 16 color palette) for any text " + "with the bold attribute. False otherwise."),
    'enable-blink': hterm.PreferenceManager.definePref_('Enable blinking text', hterm.PreferenceManager.categories.Appearance, true, 'bool', "If true, respect the blink attribute. False to ignore it."),
    'enable-clipboard-notice': hterm.PreferenceManager.definePref_('Show notification when copying content', hterm.PreferenceManager.categories.CopyPaste, true, 'bool', "Whether to show a message in the terminal when the host writes to the " + "clipboard."),
    'enable-clipboard-write': hterm.PreferenceManager.definePref_('Allow remote clipboard writes', hterm.PreferenceManager.categories.CopyPaste, true, 'bool', "Allow the remote host to write directly to the local system " + "clipboard.\n" + "Read access is never granted regardless of this setting.\n" + "\n" + "This is used to control access to features like OSC-52."),
    'enable-dec12': hterm.PreferenceManager.definePref_('Allow changing of text cursor blinking', hterm.PreferenceManager.categories.Miscellaneous, false, 'bool', "Respect the host's attempt to change the text cursor blink status " + "using DEC Private Mode 12."),
    'enable-csi-j-3': hterm.PreferenceManager.definePref_('Allow clearing of scrollback buffer (CSI-J-3)', hterm.PreferenceManager.categories.Miscellaneous, true, 'bool', "Whether CSI-J (Erase Display) mode 3 may clear the terminal " + "scrollback buffer.\n" + "\n" + "Enabling this by default is safe."),
    'environment': hterm.PreferenceManager.definePref_('Environment variables', hterm.PreferenceManager.categories.Miscellaneous, { // Signal ncurses based apps to use UTF-8 output instead of legacy
        // drawing modes (which only work in ISO-2022 mode).  Since hterm is
        // always UTF-8, this shouldn't cause problems.
        'NCURSES_NO_UTF8_ACS': '1',
        'TERM': 'xterm-256color', // Set this env var that a bunch of mainstream terminal emulators set
        // to indicate we support true colors.
        // https://gist.github.com/XVilka/8346728
        'COLORTERM': 'truecolor'
    }, 'value', "The initial set of environment variables, as an object."),
    'font-family': hterm.PreferenceManager.definePref_('Text font family', hterm.PreferenceManager.categories.Appearance, '"DejaVu Sans Mono", "Noto Sans Mono", "Everson Mono", FreeMono, ' + 'Menlo, Terminal, monospace', 'string', "Default font family for the terminal text."),
    'font-size': hterm.PreferenceManager.definePref_('Text font size', hterm.PreferenceManager.categories.Appearance, 15, 'int', "The default font size in pixels."),
    'font-smoothing': hterm.PreferenceManager.definePref_('Text font smoothing', hterm.PreferenceManager.categories.Appearance, 'antialiased', 'string', "CSS font-smoothing property."),
    'foreground-color': hterm.PreferenceManager.definePref_('Text color', hterm.PreferenceManager.categories.Appearance, 'rgb(240, 240, 240)', 'color', "The foreground color for text with no other color attributes."),
    'hide-mouse-while-typing': hterm.PreferenceManager.definePref_('Hide mouse cursor while typing', hterm.PreferenceManager.categories.Keyboard, null, 'tristate', "Whether to automatically hide the mouse cursor when typing. " + "By default, autodetect whether the platform/OS handles this.\n" + "\n" + "Note: Some operating systems may override this setting and thus you " + "might not be able to always disable it."),
    'home-keys-scroll': hterm.PreferenceManager.definePref_('Home/End key scroll behavior', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "If true, Home/End controls the terminal scrollbar and Shift-Home/" + "Shift-End are sent to the remote host. If false, then Home/End are " + "sent to the remote host and Shift-Home/Shift-End scrolls."),
    'keybindings': hterm.PreferenceManager.definePref_('Keyboard bindings/shortcuts', hterm.PreferenceManager.categories.Keyboard, null, 'value', "A map of key sequence to key actions. Key sequences include zero or " + "more modifier keys followed by a key code. Key codes can be decimal " + "or hexadecimal numbers, or a key identifier. Key actions can be " + "specified as a string to send to the host, or an action identifier. " + "For a full explanation of the format, see https://goo.gl/LWRndr.\n" + "\n" + "Sample keybindings:\n" + "{\n" + "  \"Ctrl-Alt-K\": \"clearTerminal\",\n" + "  \"Ctrl-Shift-L\": \"PASS\",\n" + "  \"Ctrl-H\": \"'Hello World'\"\n" + "}"),
    'media-keys-are-fkeys': hterm.PreferenceManager.definePref_('Media keys are Fkeys', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "If true, convert media keys to their Fkey equivalent. If false, let " + "the browser handle the keys."),
    'meta-sends-escape': hterm.PreferenceManager.definePref_('Meta key modifier handling', hterm.PreferenceManager.categories.Keyboard, true, 'bool', "Send an ESC prefix when pressing a key while holding the Meta key.\n" + "\n" + "For example, when enabled, pressing Meta-K will send ^[k as if you " + "typed Escape then k. When disabled, only k will be sent."),
    'mouse-right-click-paste': hterm.PreferenceManager.definePref_('Mouse right clicks paste content', hterm.PreferenceManager.categories.CopyPaste, true, 'bool', "Paste on right mouse button clicks.\n" + "\n" + "This option is independent of the \"mouse-paste-button\" setting.\n" + "\n" + "Note: This will handle left & right handed mice correctly."),
    'mouse-paste-button': hterm.PreferenceManager.definePref_('Mouse button paste', hterm.PreferenceManager.categories.CopyPaste, null, [null, 0, 1, 2, 3, 4, 5, 6], "Mouse paste button, or null to autodetect.\n" + "\n" + "For autodetect, we'll use the middle mouse button for non-X11 " + "platforms (including Chrome OS). On X11, we'll use the right mouse " + "button (since the native window manager should paste via the middle " + "mouse button).\n" + "\n" + "0 == left (primary) button.\n" + "1 == middle (auxiliary) button.\n" + "2 == right (secondary) button.\n" + "\n" + "This option is independent of the setting for right-click paste.\n" + "\n" + "Note: This will handle left & right handed mice correctly."),
    ...
    'word-break-match-right': hterm.PreferenceManager.definePref_('Automatic selection halting (to the right)', hterm.PreferenceManager.categories.CopyPaste, '[^\\s\\[\\](){}<>"\'\\^!@#$%&*,;:~.`]', 'string', "Regular expression to halt matching to the right (end) of a " + "selection.\n" + "\n" + "Normally this is a character class to reject specific characters."),
    'word-break-match-middle': hterm.PreferenceManager.definePref_('Word break characters', hterm.PreferenceManager.categories.CopyPaste, '[^\\s\\[\\](){}<>"\'\\^]*', 'string', "Regular expression to match all the characters in the middle.\n" + "\n" + "Normally this is a character class to reject specific characters.\n" + "\n" + "Used to expand the selection surrounding the starting point."),
    'page-keys-scroll': hterm.PreferenceManager.definePref_('Page Up/Down key scroll behavior', hterm.PreferenceManager.categories.Keyboard, false, 'bool', "If true, Page Up/Page Down controls the terminal scrollbar and " + "Shift-Page Up/Shift-Page Down are sent to the remote host. If false, " + "then Page Up/Page Down are sent to the remote host and Shift-Page Up/" + "Shift-Page Down scrolls."),
    'pass-alt-number': hterm.PreferenceManager.definePref_('Pass Alt-1..9 key behavior', hterm.PreferenceManager.categories.Keyboard, null, 'tristate', "Whether Alt-1..9 is passed to the browser.\n" + "\n" + "This is handy when running hterm in a browser tab, so that you don't " + "lose Chrome's \"switch to tab\" keyboard accelerators. When not running " + "in a tab it's better to send these keys to the host so they can be " + "used in vim or emacs.\n" + "\n" + "If true, Alt-1..9 will be handled by the browser. If false, Alt-1..9 " + "will be sent to the host. If null, autodetect based on browser " + "platform and window type."),
    'pass-ctrl-number': hterm.PreferenceManager.definePref_('Pass Ctrl-1..9 key behavior', hterm.PreferenceManager.categories.Keyboard, null, 'tristate', "Whether Ctrl-1..9 is passed to the browser.\n" + "\n" + "This is handy when running hterm in a browser tab, so that you don't " + "lose Chrome's \"switch to tab\" keyboard accelerators. When not running " + "in a tab it's better to send these keys to the host so they can be " + "used in vim or emacs.\n" + "\n" + "If true, Ctrl-1..9 will be handled by the browser. If false, " + "Ctrl-1..9 will be sent to the host. If null, autodetect based on " + "browser platform and window type."),
    ...
    'pass-meta-v': hterm.PreferenceManager.definePref_('Pass Meta-V key behavior', hterm.PreferenceManager.categories.Keyboard, true, 'bool', "Whether Meta-V gets passed to host."),
    'paste-on-drop': hterm.PreferenceManager.definePref_('Allow drag & drop to paste', hterm.PreferenceManager.categories.CopyPaste, true, 'bool', "If true, Drag and dropped text will paste into terminal.\n" + "If false, dropped text will be ignored."),
    'receive-encoding': hterm.PreferenceManager.definePref_('Receive encoding', hterm.PreferenceManager.categories.Encoding, 'utf-8', ['utf-8', 'raw'], "Set the expected encoding for data received from the host.\n" + "If the encodings do not match, visual bugs are likely to be " + "observed.\n" + "\n" + "Valid values are 'utf-8' and 'raw'."),
    'scroll-on-keystroke': hterm.PreferenceManager.definePref_('Scroll to bottom after keystroke', hterm.PreferenceManager.categories.Scrolling, true, 'bool', "Whether to scroll to the bottom on any keystroke."),
    'scroll-on-output': hterm.PreferenceManager.definePref_('Scroll to bottom after new output', hterm.PreferenceManager.categories.Scrolling, false, 'bool', "Whether to scroll to the bottom on terminal output."),
    'scrollbar-visible': hterm.PreferenceManager.definePref_('Scrollbar visibility', hterm.PreferenceManager.categories.Scrolling, true, 'bool', "The vertical scrollbar mode."),
    'scroll-wheel-may-send-arrow-keys': hterm.PreferenceManager.definePref_('Emulate arrow keys with scroll wheel', hterm.PreferenceManager.categories.Scrolling, false, 'bool', "When using the alternative screen buffer, and DECCKM (Application " + "Cursor Keys) is active, mouse wheel scroll events will emulate arrow " + "keys.\n" + "\n" + "It can be temporarily disabled by holding the Shift key.\n" + "\n" + "This frequently comes up when using pagers (less) or reading man " + "pages or text editors (vi/nano) or using screen/tmux."),
    ...
    'user-css': hterm.PreferenceManager.definePref_('Custom CSS (URI)', hterm.PreferenceManager.categories.Appearance, '', 'url', "URL of user stylesheet to include in the terminal document."),
    'user-css-text': hterm.PreferenceManager.definePref_('Custom CSS (inline text)', hterm.PreferenceManager.categories.Appearance, '', 'multiline-string', "Custom CSS text for styling the terminal."),
...
hterm.PreferenceManager.constructor = hterm.PreferenceManager; // SOURCE FILE: hterm/js/hterm_pubsub.js
// Copyright (c) 2012 The Chromium OS Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
/**
...
hterm.PubSub.prototype.publish = function(subject, e, opt_lastCallback) {
    ...
    var list = this.observers_[subject];
    if (list) { // Copy the list, in case it changes while we're notifying.
        list = [].concat(list);
    }
    if (opt_lastCallback) {
        if (list) {
...
hterm.Screen.prototype.syncSelectionCaret = function(selection) {
    ...
    } catch (firefoxIgnoredException) { // FF can throw an exception if the range is off, rather than just not
...
hterm.Screen.prototype.maybeClipCurrentRow = function() {
    ...
    if (width <= this.columnCount_) { // Current row does not need clipping, but may need clamping.
    ...
    if (this.cursorOffset_ < width - 1) {
        this.cursorNode_.textContent = hterm.TextAttributes.nodeSubstr(this.cursorNode_, 0, this.cursorOffset_ + 1);
    ...
    if (currentColumn < this.columnCount_) { // If the cursor was within the screen before we started then restore its
        // position.
...
hterm.Screen.prototype.insertString = function(str) {
    var wcwidth = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : undefined;
    var style = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "terminal-info";
    var cursorNode = this.cursorNode_;
    var cursorNodeText = cursorNode.textContent;
    this.cursorRowNode_.className = style.toString();
    this.cursorRowNode_.removeAttribute('line-overflow'); // We may alter the width of the string by prepending some missing
    ...
    if (reverseOffset < 0) { // A negative reverse offset means the cursor is positioned past the end
    ...
    if (this.textAttributes.matchesContainer(cursorNode)) { // The new text can be placed directly in the cursor node.
        if (reverseOffset == 0) {
            ...
            cursorNode.textContent = hterm.TextAttributes.nodeSubstr(cursorNode, 0, offset) + str + hterm.TextAttributes.nodeSubstr(cursorNode, offset);
        }
    ...
    if (reverseOffset == 0) { // At the end of the cursor node, the check the next sibling.
        ...
        this.cursorNode_ = newNode; // We specifically need to include any missing whitespace here, since it's
        // going in a new node.
...
hterm.Screen.prototype.insertStringStyled = function(str, style) {
    var wcwidth = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : undefined;
    var cursorNode = this.cursorNode_;
    var cursorNodeText = cursorNode.textContent;
    this.cursorRowNode_.className = style.toString();
...
hterm.Screen.prototype.overwriteString = function(str) {
    ...
    if (this.textAttributes.matchesContainer(this.cursorNode_) && this.cursorNode_.textContent.substr(this.cursorOffset_) == str) { // This overwrite would be a no-op, just move the cursor and return.
        this.cursorOffset_ += wcwidth;
        this.cursorPosition.column += wcwidth;
        return;
    }
    this.deleteChars(Math.min(wcwidth, maxLength));
    this.insertString(str, wcwidth, style.toString());
};
/**
...
hterm.Screen.prototype.deleteChars = function(count) {
    ...
    while (node && count) { // Sanity check so we don't loop forever, but we don't also go quietly.
        if (count < 0) {
            console.error("Deleting ".concat(rv, " chars went negative: ").concat(count));
            break;
        }
        ...
        endLength = hterm.TextAttributes.nodeWidth(node); // Deal with splitting wide characters.  There are two ways: we could delete
...
hterm.Screen.prototype.getLineStartRow_ = function(row) {
    while (row.previousSibling && row.previousSibling.hasAttribute('line-overflow')) {
...
hterm.Screen.prototype.getPositionWithinRow_ = function(row, node, offset) {
    if (node.parentNode != row) { // If we traversed to the top node, then there's nothing to find here.
        ...
        return this.getPositionWithinRow_(node.parentNode, node, offset) + this.getPositionWithinRow_(row, node.parentNode, 0);
    }
    var position = 0;
...
hterm.Screen.prototype.expandSelectionWithWordBreakMatches_ = function(selection, leftMatch, rightMatch, insideMatch) {
    ...
    if (endPosition == -1) return; //Move start to the left.
    ...
    if (expandedStart == -1 || expandedStart > startPosition) return; //Move end to the right.
...
hterm.ScrollPort = function(rowProvider) {
    ...
    this.rowProvider_ = rowProvider; // SWAG the character size until we can measure it.
    this.characterSize = new hterm.Size(10, 10); // DOM node used for character measurement.
    this.ruler_ = null;
    this.selection = new hterm.ScrollPort.Selection(this); // A map of rowIndex => rowNode for each row that is drawn as part of a
    ...
    this.lastScreenHeight_ = null; // True if the user should be allowed to select text in the terminal.
    ...
    this.div_ = null;
    this.document_ = null; // Collection of active timeout handles.
    this.timeouts_ = {};
    this.observers_ = {};
    this.DEBUG_ = false;
};
...
hterm.ScrollPort.Selection.prototype.sync = function() {
    ...
    if (!anchorRow) { // Don't set a selection if it's not a row node that's selected.
        return;
    }
    ...
    if (!focusRow) { // Don't set a selection if it's not a row node that's selected.
        return;
    }
    ...
    } else { // The selection starts and ends in the same row, but isn't contained all
        // in a single node.
...
hterm.ScrollPort.prototype.decorate = function(div, callback) {
    ...
    if ('mozInnerScreenX' in window) { // detect a FF only property
        this.iframe_.addEventListener('load', function() {
            return onLoad();
        });
    } else {
        onLoad();
...
hterm.ScrollPort.prototype.paintIframeContents_ = function() {
    ...
    if (this.DEBUG_) { // When we're debugging we add padding to the body so that the offscreen
        // elements are visible.
        this.document_.body.style.paddingTop = this.document_.body.style.paddingBottom = 'calc(var(--hterm-charsize-height) * 3)';
    }
    var style = doc.createElement('style');
    style.textContent = 'x-row {' + '  display: block;' + '  height: var(--hterm-charsize-height);' + '  line-height: var(--hterm-charsize-height);' + '}';
    ...
    doc.head.appendChild(this.userCssText_); // TODO(rginda): Sorry, this 'screen_' isn't the same thing as hterm.Screen
    ...
    this.screen_.setAttribute('autocapitalize', 'none'); // In some ways the terminal behaves like a text box but not in all ways. It
    ...
    this.screen_.setAttribute('aria-roledescription', 'Terminal'); // Set aria-readonly to indicate to the screen reader that the text on the
    ...
    this.scrollDownButton_.addEventListener('click', this.scrollPageDown.bind(this)); // We only allow the scroll buttons to display after a delay, otherwise the
    ...
    setTimeout(function() {
        _this9.allowScrollButtonsToDisplay_ = true;
    }, 500);
    this.document_.addEventListener('selectionchange', function() {
        _this9.selection.sync();
        if (!_this9.allowScrollButtonsToDisplay_) return;
        var accessibilityEnabled = _this9.accessibilityReader_ && _this9.accessibilityReader_.accessibilityEnabled;
        var selection = _this9.document_.getSelection();
        var selectedElement;
        if (selection.anchorNode && selection.anchorNode.parentElement) {
            selectedElement = selection.anchorNode.parentElement;
        }
        if (accessibilityEnabled && selectedElement == _this9.scrollUpButton_) {
            _this9.scrollUpButton_.style.top = '0px';
        } else {
            _this9.scrollUpButton_.style.top = -scrollButtonTotalHeight + 'px';
        }
        if (accessibilityEnabled && selectedElement == _this9.scrollDownButton_) {
            _this9.scrollDownButton_.style.bottom = '0px';
        } else {
            _this9.scrollDownButton_.style.bottom = -scrollButtonTotalHeight + 'px';
        }
    });
    this.screen_.appendChild(this.scrollUpButton_); // This is the main container for the fixed rows.
    ...
    this.screen_.appendChild(this.scrollDownButton_); // Two nodes to hold offscreen text during the copy event.
    ...
    this.bottomSelectBag_ = this.topSelectBag_.cloneNode(); // Nodes above the top fold and below the bottom fold are hidden.  They are
    ...
    this.screen_.appendChild(this.scrollArea_); // This svg element is used to detect when the browser is zoomed.  It must be
    ...
    this.svg_.style.cssText = 'position: absolute;' + 'top: 0;' + 'left: 0;' + 'visibility: hidden'; // We send focus to this element just before a paste happens, so we can
...
hterm.ScrollPort.prototype.measureCharacterSize = function(opt_weight) { // Number of lines used to average the height of a single character.
    ...
    if (!this.ruler_) {
        ...
        this.ruler_.style.cssText = 'position: absolute;' + 'top: 0;' + 'left: 0;' + 'visibility: hidden;' + 'height: auto !important;' + 'width: auto !important;'; // We need to put the text in a span to make the size calculation
        // work properly in Firefox
        this.rulerSpan_ = this.document_.createElement('span');
        this.rulerSpan_.id = 'hterm:ruler-span-workaround';
        this.rulerSpan_.innerHTML = ('X'.repeat(lineLength) + '\r').repeat(numberOfLines);
        this.ruler_.appendChild(this.rulerSpan_);
        this.rulerBaseline_ = this.document_.createElement('span');
        this.rulerSpan_.id = 'hterm:ruler-baseline'; // We want to collapse it on the baseline
    ...
    var size = new hterm.Size(rulerSize.width / lineLength, rulerSize.height / numberOfLines);
...
hterm.ScrollPort.prototype.assertiveAnnounce_ = function() {
    ...
    var topRow = this.getTopRowIndex();
    var bottomRow = this.getBottomRowIndex(topRow);
    var percentScrolled = 100 * topRow / Math.max(1, this.rowProvider_.getRowCount() - this.visibleRowCount);
...
hterm.ScrollPort.prototype.syncRowNodesDimensions_ = function() {
    ...
    this.lastScreenHeight_ = screenSize.height; // We don't want to show a partial row because it would be distracting
...
hterm.ScrollPort.prototype.updateScrollButtonState_ = function() {
    var setButton = function setButton(button, disabled) {
...
hterm.ScrollPort.prototype.drawTopFold_ = function(topRowIndex) {
    ...
    if (!this.selection.isMultiline || this.selection.endRow.rowIndex >= topRowIndex) { // Only the startRow is above the fold.
        ...
        if (this.selection.endRow.nextSibling != this.topFold_) {
            this.rowNodes_.insertBefore(this.topFold_, this.selection.endRow.nextSibling);
...
hterm.ScrollPort.prototype.drawVisibleRows_ = function(topRowIndex, bottomRowIndex) {
    ...
    var bottomFold = this.bottomFold_; // The node we're examining during the current iteration.
    var node = this.topFold_.nextSibling;
    ...
    for (var drawCount = 0; drawCount < targetDrawCount; drawCount++) {
        ...
        if (node == bottomFold) { // We've hit the bottom fold, we need to insert a new row.
...
hterm.ScrollPort.prototype.onScroll_ = function(e) {
    var screenSize = this.getScreenSize();
    if (screenSize.width != this.lastScreenWidth_ || screenSize.height != this.lastScreenHeight_) { // This event may also fire during a resize (but before the resize event!).
...
hterm.ScrollPort.prototype.onScrollWheel_ = function(e) {
    ...
    if (e.defaultPrevented) return; // Figure out how far this event wants us to scroll.
    var delta = this.scrollWheelDelta(e);
    var top = this.screen_.scrollTop - delta.y;
    if (top < 0) top = 0;
    var scrollMax = this.getScrollMax_();
    if (top > scrollMax) top = scrollMax;
    if (top != this.screen_.scrollTop) { // Moving scrollTop causes a scroll event, which triggers the redraw.
...
hterm.ScrollPort.prototype.onTouch_ = function(e) {
    this.onTouch(e);
    if (e.defaultPrevented) return; // Extract the fields from the Touch event that we need.  If we saved the
    ...
    var scrubTouch = function scrubTouch(t) {
        return {
            id: t.identifier,
            y: t.clientY,
            x: t.clientX
    ...
    var i, touch;
    switch (e.type) {
        case 'touchstart': // Workaround focus bug on CrOS if possible.
            ...
            break;
        case 'touchcancel':
        case 'touchend': // Throw away existing touches that we're finished with.
            for (i = 0; i < e.changedTouches.length; ++i) {
                delete this.lastTouch_[e.changedTouches[i].identifier];
            }
            break;
        case 'touchmove': // Walk all of the touches in this one event and merge all of their
...
hterm.ScrollPort.prototype.onCopy_ = function(e) {
    ...
    this.resetSelectBags_();
    this.selection.sync();
    if (this.selection.isCollapsed || this.selection.endRow.rowIndex - this.selection.startRow.rowIndex < 2) {
        return;
    }
    var topRowIndex = this.getTopRowIndex();
    var bottomRowIndex = this.getBottomRowIndex(topRowIndex);
    if (this.selection.startRow.rowIndex < topRowIndex) { // Start of selection is above the top fold.
        var endBackfillIndex;
        if (this.selection.endRow.rowIndex < topRowIndex) { // Entire selection is above the top fold.
    ...
    if (this.selection.endRow.rowIndex > bottomRowIndex) { // Selection ends below the bottom fold.
        ...
        if (this.selection.startRow.rowIndex > bottomRowIndex) { // Entire selection is below the bottom fold.
...
hterm.ScrollPort.prototype.onBodyKeyDown_ = function(e) {
    ...
    if ((e.ctrlKey || e.metaKey) && e.keyCode == 86 /* 'V' */ ) this.pasteTarget_.focus();
};
/**
...
hterm.ScrollPort.prototype.onDragAndDrop_ = function(e) {
    if (!this.pasteOnDrop) return;
    e.preventDefault();
    var data;
    var format; // If the shift key active, try to find a "rich" text source (but not plain
    // text).  e.g. text/html is OK.
    if (e.shiftKey) {
        e.dataTransfer.types.forEach(function(t) {
...
hterm.Terminal = function(opt_profileId) {
    ...
    this.alternateScreen_ = new hterm.Screen(); // The "current" screen.
    ...
    this.scrollPort_.onCopy = this.onCopy_.bind(this); // The div that contains this terminal.
    ...
    this.vtScrollBottom_ = null; // The DIV element for the visible cursor.
    ...
    this.scrollWheelArrowKeys_ = null; // True if we should override mouse event reporting to allow local selection.
    ...
    this.bellAudio_.setAttribute('preload', 'auto'); // The AccessibilityReader object for announcing command output.
    ...
    this.saveCursorAndState(true); // The keyboard handler.
    ...
    this.mousePasteButton = null; // Whether to use the default window copy behavior.
    this.useDefaultWindowCopy = false;
    this.clearSelectionAfterCopy = true;
    this.realizeSize_(80, 24);
    this.setDefaultTabStops(); // Whether we allow images to be shown.
...
hterm.Terminal.prototype.setProfile = function(profileId, opt_callback) {
    ...
    this.prefs_.addObservers(null, {
        'alt-gr-mode': function altGrMode(v) {
            if (v == null) {
        ...
        'alt-backspace-is-meta-backspace': function altBackspaceIsMetaBackspace(v) {
            terminal.keyboard.altBackspaceIsMetaBackspace = v;
        },
        'alt-is-meta': function altIsMeta(v) {
            terminal.keyboard.altIsMeta = v;
        },
        'alt-sends-what': function altSendsWhat(v) {
            if (!/^(escape|8-bit|browser-key)$/.test(v)) v = 'escape';
            terminal.keyboard.altSendsWhat = v;
        },
        'audible-bell-sound': function audibleBellSound(v) {
            var ary = v.match(/^lib-resource:(\S+)/);
            if (ary) {
                terminal.bellAudio_.setAttribute('src', lib.resource.getDataUrl(ary[1]));
            } else {
                terminal.bellAudio_.setAttribute('src', v);
            }
        },
        'desktop-notification-bell': function desktopNotificationBell(v) {
            if (v && Notification) {
                terminal.desktopNotificationBell_ = Notification.permission === 'granted';
                if (!terminal.desktopNotificationBell_) { // Note: We don't call Notification.requestPermission here because
        ...
        'background-color': function backgroundColor(v) {
            terminal.setBackgroundColor(v);
        },
        'background-image': function backgroundImage(v) {
            terminal.scrollPort_.setBackgroundImage(v);
        },
        'background-size': function backgroundSize(v) {
            terminal.scrollPort_.setBackgroundSize(v);
        },
        'background-position': function backgroundPosition(v) {
            terminal.scrollPort_.setBackgroundPosition(v);
        },
        'backspace-sends-backspace': function backspaceSendsBackspace(v) {
            terminal.keyboard.backspaceSendsBackspace = v;
        },
        'character-map-overrides': function characterMapOverrides(v) {
            if (!(v == null || _instanceof(v, Object))) {
        ...
        'cursor-blink': function cursorBlink(v) {
            terminal.setCursorBlink(!!v);
        },
        'cursor-blink-cycle': function cursorBlinkCycle(v) {
            if (_instanceof(v, Array) && typeof v[0] == 'number' && typeof v[1] == 'number') {
                terminal.cursorBlinkCycle_ = v;
            } else if (typeof v == 'number') {
                terminal.cursorBlinkCycle_ = [v, v];
            } else { // Fast blink indicates an error.
                terminal.cursorBlinkCycle_ = [100, 100];
            }
        },
        'cursor-color': function cursorColor(v) {
            terminal.setCursorColor(v);
        },
        'color-palette-overrides': function colorPaletteOverrides(v) {
            if (!(v == null || _instanceof(v, Object) || _instanceof(v, Array))) {
        ...
        'copy-on-select': function copyOnSelect(v) {
            terminal.copyOnSelect = !!v;
        },
        'use-default-window-copy': function useDefaultWindowCopy(v) {
            terminal.useDefaultWindowCopy = !!v;
        },
        'clear-selection-after-copy': function clearSelectionAfterCopy(v) {
            terminal.clearSelectionAfterCopy = !!v;
        },
        'ctrl-plus-minus-zero-zoom': function ctrlPlusMinusZeroZoom(v) {
            terminal.keyboard.ctrlPlusMinusZeroZoom = v;
        },
        'ctrl-c-copy': function ctrlCCopy(v) {
            terminal.keyboard.ctrlCCopy = v;
        },
        'ctrl-v-paste': function ctrlVPaste(v) {
            terminal.keyboard.ctrlVPaste = v;
            terminal.scrollPort_.setCtrlVPaste(v);
        },
        'paste-on-drop': function pasteOnDrop(v) {
            terminal.scrollPort_.setPasteOnDrop(v);
        },
        'east-asian-ambiguous-as-two-column': function eastAsianAmbiguousAsTwoColumn(v) {
            lib.wc.regardCjkAmbiguous = v;
        },
        'enable-8-bit-control': function enable8BitControl(v) {
            terminal.vt.enable8BitControl = !!v;
        },
        'enable-bold': function enableBold(v) {
            terminal.syncBoldSafeState();
        },
        'enable-bold-as-bright': function enableBoldAsBright(v) {
            terminal.primaryScreen_.textAttributes.enableBoldAsBright = !!v;
            terminal.alternateScreen_.textAttributes.enableBoldAsBright = !!v;
        },
        'enable-blink': function enableBlink(v) {
            terminal.setTextBlink(!!v);
        },
        'enable-clipboard-write': function enableClipboardWrite(v) {
            terminal.vt.enableClipboardWrite = !!v;
        },
        'enable-dec12': function enableDec12(v) {
            terminal.vt.enableDec12 = !!v;
        },
        'enable-csi-j-3': function enableCsiJ3(v) {
            terminal.vt.enableCsiJ3 = !!v;
        },
        'font-family': function fontFamily(v) {
            terminal.syncFontFamily();
        },
        'font-size': function fontSize(v) {
            v = parseInt(v);
            if (v <= 0) {
                console.error("Invalid font size: ".concat(v));
                return;
            }
            terminal.setFontSize(v);
        },
        'font-smoothing': function fontSmoothing(v) {
            terminal.syncFontFamily();
        },
        'foreground-color': function foregroundColor(v) {
            terminal.setForegroundColor(v);
        },
        'hide-mouse-while-typing': function hideMouseWhileTyping(v) {
            terminal.setAutomaticMouseHiding(v);
        },
        'home-keys-scroll': function homeKeysScroll(v) {
            terminal.keyboard.homeKeysScroll = v;
        },
        'keybindings': function keybindings(v) {
            terminal.keyboard.bindings.clear();
            if (!v) return;
            if (!_instanceof(v, Object)) {
        ...
        'media-keys-are-fkeys': function mediaKeysAreFkeys(v) {
            terminal.keyboard.mediaKeysAreFKeys = v;
        },
        'meta-sends-escape': function metaSendsEscape(v) {
            terminal.keyboard.metaSendsEscape = v;
        },
        'mouse-right-click-paste': function mouseRightClickPaste(v) {
            terminal.mouseRightClickPaste = v;
        },
        'mouse-paste-button': function mousePasteButton(v) {
            terminal.syncMousePasteButton();
        },
        'page-keys-scroll': function pageKeysScroll(v) {
            terminal.keyboard.pageKeysScroll = v;
        },
        'pass-alt-number': function passAltNumber(v) {
        ...
        'pass-meta-v': function passMetaV(v) {
            terminal.keyboard.passMetaV = v;
        },
        'receive-encoding': function receiveEncoding(v) {
            if (!/^(utf-8|raw)$/.test(v)) {
        ...
        'scroll-on-keystroke': function scrollOnKeystroke(v) {
            terminal.scrollOnKeystroke_ = v;
        },
        'scroll-on-output': function scrollOnOutput(v) {
            terminal.scrollOnOutput_ = v;
        },
        'scrollbar-visible': function scrollbarVisible(v) {
            terminal.setScrollbarVisible(v);
        },
        'scroll-wheel-may-send-arrow-keys': function scrollWheelMaySendArrowKeys(v) {
            terminal.scrollWheelArrowKeys_ = v;
        },
        'scroll-wheel-move-multiplier': function scrollWheelMoveMultiplier(v) {
            terminal.setScrollWheelMoveMultipler(v);
        },
        'shift-insert-paste': function shiftInsertPaste(v) {
            terminal.keyboard.shiftInsertPaste = v;
        },
        'terminal-encoding': function terminalEncoding(v) {
            terminal.vt.setEncoding(v);
        },
        'user-css': function userCss(v) {
            terminal.scrollPort_.setUserCssUrl(v);
        },
        'user-css-text': function userCssText(v) {
            terminal.scrollPort_.setUserCssText(v);
        },
        'word-break-match-left': function wordBreakMatchLeft(v) {
            terminal.primaryScreen_.wordBreakMatchLeft = v;
            terminal.alternateScreen_.wordBreakMatchLeft = v;
        },
        'word-break-match-right': function wordBreakMatchRight(v) {
            terminal.primaryScreen_.wordBreakMatchRight = v;
            terminal.alternateScreen_.wordBreakMatchRight = v;
        },
        'word-break-match-middle': function wordBreakMatchMiddle(v) {
            terminal.primaryScreen_.wordBreakMatchMiddle = v;
            terminal.alternateScreen_.wordBreakMatchMiddle = v;
        },
        'allow-images-inline': function allowImagesInline(v) {
...
hterm.Terminal.prototype.setCursorColor = function(color) {
    if (color === undefined) color = this.prefs_.get('cursor-color');
...
hterm.Terminal.prototype.setBackgroundColor = function(color) {
    if (color === undefined) color = this.prefs_.get('background-color');
...
hterm.Terminal.prototype.setForegroundColor = function(color) {
    if (color === undefined) color = this.prefs_.get('foreground-color');
...
hterm.Terminal.prototype.runCommandClass = function(commandClass, argString) {
    ...
    if (_typeof(environment) != 'object' || environment == null) environment = {};
    var self = this;
    this.command = new commandClass({
        argString: argString || '',
        io: this.io.push(),
        environment: environment,
        onExit: function onExit(code) {
            self.io.pop();
...
hterm.Terminal.prototype.setCssVar = function(name, value) {
    var opt_prefix = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '--hterm-';
    this.document_.documentElement.style.setProperty("".concat(opt_prefix).concat(name), value);
};
/**
 * Get a CSS variable.
 *
 ...
 */
hterm.Terminal.prototype.getCssVar = function(name) {
    var opt_prefix = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '--hterm-';
    return this.document_.documentElement.style.getPropertyValue("".concat(opt_prefix).concat(name));
};
/**
...
hterm.Terminal.prototype.syncBoldSafeState = function() {
    ...
    if (!isBoldSafe) {
        console.warn('Bold characters disabled: Size of bold weight differs ' + 'from normal.  Font family is: ' + this.scrollPort_.getFontFamily());
    }
...
hterm.Terminal.prototype.setTextBlink = function(state) {
    if (state === undefined) state = this.prefs_.get('enable-blink');
...
hterm.Terminal.prototype.saveCursorAndState = function(both) {
    ...
    } else this.screen_.saveCursorAndState(this.vt);
};
/**
 * Restore the saved cursor state in the corresponding screens.
 *
...
hterm.Terminal.prototype.restoreCursorAndState = function(both) {
    ...
    } else this.screen_.restoreCursorAndState(this.vt);
};
/**
...
hterm.Terminal.prototype.realizeSize_ = function(columnCount, rowCount) {
    ...
    if (rowCount != this.screenSize.height) this.realizeHeight_(rowCount); // Send new terminal size to plugin.
...
hterm.Terminal.prototype.realizeHeight_ = function(rowCount) {
    ...
    if (deltaRows < 0) { // Screen got smaller.
        deltaRows *= -1;
        while (deltaRows) {
        ...
        this.scrollbackRows_.push.apply(this.scrollbackRows_, ary); // We just removed rows from the top of the screen, we need to update
        // the cursor to match.
...
hterm.Terminal.prototype.clearScrollback = function() {
    var _this10 = this; // Move to the end of the buffer in case the screen was scrolled back.
    ...
    [this.primaryScreen_, this.alternateScreen_].forEach(function(screen) {
        ...
        _this10.renumberRows_(0, bottom, screen);
    });
...
hterm.Terminal.prototype.reset = function() {
    ...
    var resetScreen = function resetScreen(screen) { // We want to make sure to reset the attributes before we clear the screen.
        ...
        _this11.clearHome(screen);
        screen.saveCursorAndState(_this11.vt);
    };
    resetScreen(this.primaryScreen_);
    resetScreen(this.alternateScreen_); // Reset terminal options to their default values.
...
hterm.Terminal.prototype.softReset = function() {
    var _this12 = this;
    this.vt.reset(); // Reset terminal options to their default values.
    ...
    var resetScreen = function resetScreen(screen) { // Xterm also resets the color palette on soft reset, even though it doesn't
        // seem to be documented anywhere.
        screen.textAttributes.reset();
        screen.textAttributes.resetColorPalette();
        screen.saveCursorAndState(_this12.vt);
    };
    resetScreen(this.primaryScreen_);
    resetScreen(this.alternateScreen_); // The xterm man page explicitly says this will happen on soft reset.
...
hterm.Terminal.prototype.setDefaultTabStops = function(opt_start) {
    ...
    var w = this.tabWidth; // Round start up to a default tab stop.
...
hterm.Terminal.prototype.decorate = function(div) {
    var _this13 = this;
    var charset = div.ownerDocument.characterSet.toLowerCase();
    if (charset != 'utf-8') {
        console.warn("Document encoding should be set to utf-8, not \"".concat(charset, "\";") + " Add <meta charset='utf-8'/> to your HTML <head> to fix.");
    }
    this.div_ = div;
    this.accessibilityReader_ = new hterm.AccessibilityReader(div);
    this.scrollPort_.decorate(div, function() {
        return _this13.setupScrollPort_();
    });
};
/**
...
hterm.Terminal.prototype.setupScrollPort_ = function() {
    ...
    screenNode.addEventListener('focus', this.onFocusChange_.bind(this, true)); // Listen for mousedown events on the screenNode as in FF the focus
    // events don't bubble.
    ...
    this.setCursorBlink(!!this.prefs_.get('cursor-blink'));
    this.restyleCursor_();
    this.document_.body.appendChild(this.cursorNode_); // When 'enableMouseDragScroll' is off we reposition this element directly
...
hterm.Terminal.prototype.appendRows_ = function(count) {
    ...
    if (cursorRow >= this.screen_.rowsArray.length) cursorRow = this.screen_.rowsArray.length - 1;
 ...
 */
 hterm.Terminal.prototype.setStyle = function(style){
    this.style = style;
 }

 hterm.Terminal.prototype.setDrc = function(drc, drcpointer){
    this.drc = drc;
    this.drcpointer = drcpointer;
}

hterm.Terminal.prototype.print = function(str) {
    this.scheduleSyncCursorPosition_(); // Basic accessibility output for the screen reader.
    this.accessibilityReader_.announce(str);
    var startOffset = 0;
    var strWidth = lib.wc.strWidth(str); // Fun edge case: If the string only contains zero width codepoints (like
    ...
    while (startOffset < strWidth) {
        ...
        if (didOverflow && !this.options_.wraparound) { // If the string overflowed the line but wraparound is off, then the
            ...
            substr = lib.wc.substr(str, startOffset, count - 1) + lib.wc.substr(str, strWidth - 1);
            count = strWidth;
        } else {
        ...
        for (var i = 0; i < tokens.length; i++) {
            ...
            if (this.options_.insertMode) {
                this.screen_.insertString(tokens[i].str, tokens[i].wcStrWidth, this.style);
            } else {
                this.screen_.overwriteString(tokens[i].str, tokens[i].wcStrWidth, this.style);
            }
            ...
            if(i == 0 && this.drc){
                ...
                var inpt = t.document_.createElement('input');
                inpt.setAttribute("type", "image");
                inpt.setAttribute("src", "data:image/png;base64,");
                //inpt.setAttribute("class", "drc");
                var p = this.drcpointer;
                inpt.onclick = function(){
                    JSBridge.generateDrcTrace(p);
                }

               // var btn = t.document_.createElement('button');
               // btn.setAttribute("class", "drc");
               // var p = this.drcpointer;
              //  btn.onclick = function(){
               //    JSBridge.generateDrcTrace(p);
              //  }
                //JSBridge.generateDrcTrace(pointer);
                this.screen_.cursorRowNode_.insertBefore(inpt, this.screen_.cursorRowNode_.firstChild)
                this.drc = false;
                this.drcpointer = -1;
            }
...
hterm.Terminal.prototype.newLine = function() {
    var dueToOverflow = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
    if (!dueToOverflow) this.accessibilityReader_.newLine();
    var cursorAtEndOfScreen = this.screen_.cursorPosition.row == this.screen_.rowsArray.length - 1;
    if (this.vtScrollBottom_ != null) { // A VT Scroll region is active, we never append new rows.
...
hterm.Terminal.prototype.clearHome = function(opt_screen) {
    ...
    if (bottom == 0) { // Empty screen, nothing to do.
        return;
    }
...
hterm.Terminal.prototype.insertLines = function(count) {
    ...
    count = Math.min(count, bottom - cursorRow); // The moveCount is the number of rows we need to relocate to make room for
...
hterm.Terminal.prototype.cursorLeft = function(count) {
    ...
    if (this.options_.reverseWraparound) {
        ...
        if (newColumn < 0) {
            ...
            if (newRow < 0) { // xterm also wraps from row 0 to the last row.
...
hterm.Terminal.prototype.ringBell = function() {
    ...
    setTimeout(function() {
        self.restyleCursor_();
    }, 200); // bellSquelchTimeout_ affects both audio and notification bells.
    ...
    if (this.desktopNotificationBell_ && !this.document_.hasFocus()) {
        var n = hterm.notify();
        this.bellNotificationList_.push(n); // TODO: Should we try to raise the window here?
        n.onclick = function() {
...
hterm.Terminal.prototype.setAlternateMode = function(state) {
    ...
    if (this.screen_.rowsArray.length && this.screen_.rowsArray[0].rowIndex != this.scrollbackRows_.length) { // If the screen changed sizes while we were away, our rowIndexes may
        // be incorrect.
        var offset = this.scrollbackRows_.length;
...
hterm.Terminal.prototype.syncCursorPosition_ = function() {
    ...
    var forceSyncSelection = false;
    if (this.accessibilityReader_.accessibilityEnabled) { // Report the new position of the cursor for accessibility purposes.
        var cursorColumnIndex = this.screen_.cursorPosition.column;
        var cursorLineText = this.screen_.rowsArray[this.screen_.cursorPosition.row].innerText; // This will force the selection to be sync'd to the cursor position if the
    ...
    if (cursorRowIndex > bottomRowIndex) { // Cursor is scrolled off screen, move it outside of the visible area.
    ...
    this.cursorNode_.setAttribute('title', '(' + this.screen_.cursorPosition.column + ', ' + this.screen_.cursorPosition.row + ')'); // Update the caret for a11y purposes.
    var selection = this.document_.getSelection();
...
hterm.Terminal.prototype.restyleCursor_ = function() {
    ...
    if (this.cursorNode_.getAttribute('focus') == 'false') { // Always show a block cursor when unfocused.
    ...
    switch (shape) {
        ...
        case hterm.Terminal.cursorShape.UNDERLINE:
            ...
            style.backgroundColor = 'transparent';
            style.borderBottomStyle = 'solid'; // correct the size to put it exactly at the baseline
...
hterm.Terminal.prototype.scheduleSyncCursorPosition_ = function() {
    ...
    if (this.accessibilityReader_.accessibilityEnabled) { // Report the previous position of the cursor for accessibility purposes.
        var cursorRowIndex = this.scrollbackRows_.length + this.screen_.cursorPosition.row;
        var cursorColumnIndex = this.screen_.cursorPosition.column;
        var cursorLineText = this.screen_.rowsArray[this.screen_.cursorPosition.row].innerText;
        this.accessibilityReader_.beforeCursorChange(cursorLineText, cursorRowIndex, cursorColumnIndex);
    }
    var self = this;
...
hterm.Terminal.prototype.showZoomWarning_ = function(state) {
    ...
    this.zoomWarningNode_.textContent = lib.i18n.replaceReferences(hterm.zoomWarningMessage, [parseInt(this.scrollPort_.characterSize.zoomFactor * 100)]);
...
hterm.Terminal.prototype.showOverlay = function(msg, opt_timeout) {
    ...
    this.overlayTimeout_ = setTimeout(function() {
        _this14.overlayNode_.style.opacity = '0';
        _this14.overlayTimeout_ = setTimeout(function() {
            return _this14.hideOverlay();
        }, 200);
    }, opt_timeout || 1500);
};
/**
...
hterm.Terminal.prototype.hideOverlay = function() {
    ...
    if (this.overlayNode_.parentNode) this.overlayNode_.parentNode.removeChild(this.overlayNode_);
    this.overlayNode_.style.opacity = '0.75';
};
/**
...
hterm.Terminal.prototype.displayImage = function(options, onLoad, onError) {
    var _this15 = this; // Make sure we're actually given a resource to display.
    ...
    if (!options.type) {
        var ary = options.name.split('.');
        var ext = ary[ary.length - 1].trim();
        switch (ext) {
            case 'svg':
            case 'svgz':
    ...
    if (this.allowImagesInline !== true) {
        ...
        button.addEventListener('click', function() {
            _this15.prefs_.set('allow-images-inline', false);
        });
        ...
        button.style.borderStyle = 'solid';
        button.addEventListener('click', function() {
            _this15.allowImagesInline = true;
        });
        span.appendChild(button);
        button = this.document_.createElement('span');
        ...
        button.style.borderStyle = 'solid';
        button.addEventListener('click', function() {
            _this15.prefs_.set('allow-images-inline', true);
        });
        span.appendChild(button);
    ...
    if (options.inline) {
        var io = this.io.push();
        io.showOverlay(hterm.msg('LOADING_RESOURCE_START', [options.name], 'Loading $1 ...'), null); // While we're loading the image, eat all the user's input.
        io.onVTKeystroke = io.sendString = function() {}; // Initialize this new image.
        var img = /** @type {!HTMLImageElement} */ this.document_.createElement('img');
        if (options.uri !== undefined) {
            img.src = options.uri;
        } else if (options.buffer !== undefined) {
            var blob = new Blob([options.buffer], {
                type: options.type
            });
            img.src = URL.createObjectURL(blob);
        } else {
            var _blob = new Blob([options.blob], {
                type: options.type
            });
            img.src = URL.createObjectURL(options.blob);
        }
        img.title = img.alt = options.name; // Attach the image to the page to let it load/render.  It won't stay here.
        ...
        img.onload = function() { // Now that we have the image dimensions, figure out how to show it.
            ...
            img.style.maxWidth = "".concat(_this15.document_.body.clientWidth, "px");
            img.style.maxHeight = "".concat(_this15.document_.body.clientHeight, "px"); // Parse a width/height specification.
            var parseDim = function parseDim(dim, maxDim, cssVar) {
                if (!dim || dim == 'auto') return '';
                var ary = dim.match(/^([0-9]+)(px|%)?$/);
                if (ary) {
                    if (ary[2] == '%') return maxDim * parseInt(ary[1]) / 100 + 'px';
                    else if (ary[2] == 'px') return dim;
                    else return "calc(".concat(dim, " * var(").concat(cssVar, "))");
                }
                return '';
            };
            img.style.width = parseDim(options.width, _this15.document_.body.clientWidth, '--hterm-charsize-width');
            img.style.height = parseDim(options.height, _this15.document_.body.clientHeight, '--hterm-charsize-height'); // Figure out how many rows the image occupies, then add that many.
            // XXX: This count will be inaccurate if the font size changes on us.
            var padRows = Math.ceil(img.clientHeight / _this15.scrollPort_.characterSize.height);
            ...
            var div = _this15.document_.createElement('div');
            div.style.position = 'relative';
            div.style.textAlign = options.align;
            img.style.position = 'absolute';
            img.style.bottom = 'calc(0px - var(--hterm-charsize-height))';
            div.appendChild(img);
            var row = _this15.getRowNode(_this15.scrollbackRows_.length + _this15.getCursorRow() - 1);
            row.appendChild(div); // Now that the image has been read, we can revoke the source.
            if (options.uri === undefined) {
        ...
        img.onerror = function(e) {
            _this15.document_.body.removeChild(img);
        ...
        if (options.uri !== undefined) {
            ...
            var _blob2 = new Blob([options.buffer]);
            a.href = URL.createObjectURL(_blob2);
        } else {
...
hterm.Terminal.prototype.getSelectionText = function() {
    ...
    selection.sync();
    if (selection.isCollapsed) return null; // Start offset measures from the beginning of the line.
    var startOffset = selection.startOffset;
    var node = selection.startNode; // If an x-row isn't selected, |node| will be null.
    if (!node) return null;
    if (node.nodeName != 'X-ROW') { // If the selection doesn't start on an x-row node, then it must be
    ...
    if (node.nodeName != 'X-ROW') { // If the selection doesn't end on an x-row node, then it must be
...
hterm.Terminal.prototype.openSelectedUrl_ = function() {
    ...
    if (str == null) {
        ...
        str = this.getSelectionText(); // If clicking in empty space, return.
        if (str == null) return;
...
hterm.Terminal.prototype.setAutomaticMouseHiding = function() {
    ...
    if (v === null) v = hterm.os != 'cros' && hterm.os != 'mac';
...
hterm.Terminal.prototype.onMouse_ = function(e) {
    var _this16 = this;
    if (e.processedByTerminalHandler_) { // We register our event handlers on the document, as well as the cursor
    ...
    if (e.button > 2) {
        e.preventDefault(); // We don't return so click events can be passed to the remote below.
    }
    var reportMouseEvents = !this.defeatMouseReports_ && this.vt.mouseReport != this.vt.MOUSE_REPORT_DISABLED;
    e.processedByTerminalHandler_ = true; // Handle auto hiding of mouse cursor while typing.
    if (this.mouseHideWhileTyping_ && !this.mouseHideDelay_) { // Make sure the mouse cursor is visible.
        ...
        this.mouseHideDelay_ = setTimeout(function() {
            return _this16.mouseHideDelay_ = null;
        }, 1000);
    ...
    if (e.type == 'mousedown' && e.terminalColumn > this.screenSize.width) { // Mousedown in the scrollbar area.
        return;
    }
    if (this.options_.cursorVisible && !reportMouseEvents) { // If the cursor is visible and we're not sending mouse events to the
        ...
        if (e.terminalRow - 1 == this.screen_.cursorPosition.row && e.terminalColumn - 1 == this.screen_.cursorPosition.column) {
    ...
    if (!reportMouseEvents) {
        ...
        if (e.type == 'click' && !e.shiftKey && (e.ctrlKey || e.metaKey)) { // Debounce this event with the dblclick event.  If you try to doubleclick
        ...
        if (e.type == 'mousedown') {
            ...
            } else if (e.button == this.mousePasteButton || this.mouseRightClickPaste && e.button == 2 /* right button */ ) {
                if (!this.paste()) console.warn('Could not paste manually due to web restrictions');
            }
        }
        if (e.type == 'mouseup' && e.button == 0 && this.copyOnSelect && !this.document_.getSelection().isCollapsed) {
            this.copySelectionToClipboard();
        }
        if ((e.type == 'mousemove' || e.type == 'mouseup') && this.scrollBlockerNode_.engaged) { // Disengage the scroll-blocker after one of these events.
        ...
        if (this.scrollWheelArrowKeys_ && !e.shiftKey && this.keyboard.applicationCursor && !this.isPrimaryScreen()) {
            if (e.type == 'wheel') {
                var delta = this.scrollPort_.scrollWheelDelta(e); // Helper to turn a wheel event delta into a series of key presses.
                var deltaToArrows = function deltaToArrows(distance, charSize, arrowPos, arrowNeg) {
                    if (distance == 0) {
                        return '';
                    } // Convert the scroll distance into a number of rows/cols.
                    var cells = lib.f.smartFloorDivide(Math.abs(distance), charSize);
                    var data = '\x1bO' + (distance < 0 ? arrowNeg : arrowPos);
                ...
                this.io.sendString( // Up/down arrow keys.
                    ...
                    deltaToArrows(delta.x, this.scrollPort_.characterSize.width, 'C', 'D'));
                e.preventDefault();
            }
        }
...
hterm.Terminal.prototype.onFocusChange_ = function(focused) {
    ...
    this.restyleCursor_();
    if (this.reportFocus) this.io.sendString(focused === true ? '\x1b[I' : '\x1b[O');
...
hterm.Terminal.prototype.onPaste_ = function(e) {
    ...
    if (this.options_.bracketedPaste) { // We strip out most escape sequences as they can cause issues (like
...
hterm.Terminal.prototype.onResize_ = function() {
    ...
    if (columnCount <= 0 || rowCount <= 0) { // We avoid these situations since they happen sometimes when the terminal
...
hterm.Terminal.IO.prototype.writeUTF8 = function(string) { // We can't use instanceof here on string to see if it's an ArrayBuffer as it
    ...
    if (typeof string == 'string') {
        if (this.terminal_.characterEncoding != 'raw') {
            var bytes = lib.codec.stringToCodeUnitArray(string, Uint8Array);
            string = this.textDecoder_.decode(bytes, {
                stream: true
            });
        }
    } else { // Handle array buffers & typed arrays by normalizing into a typed array.
        var u8 = new Uint8Array(string);
        if (this.terminal_.characterEncoding == 'raw') {
            string = lib.codec.codeUnitArrayToString(u8);
        } else {
            string = this.textDecoder_.decode(u8, {
                stream: true
            });
        }
    }
...
hterm.Terminal.IO.prototype.writelnUTF8 = function(string) {
    this.writeUTF8(string); // We need to use writeUTF8 to make sure we flush the decoder state.
    this.writeUTF8('\r\n');
};
/**
...
hterm.Terminal.IO.prototype.printlnStyled = hterm.Terminal.IO.prototype.writelnUTF16Styled = function(string, style) {
    this.terminal_.setStyle(style);
    this.print(string + '\r\n');
};

hterm.Terminal.IO.prototype.printlnDrc = hterm.Terminal.IO.prototype.writelnUTF16Drc = function(string, style, drcpointer) {
    this.terminal_.setStyle(style);
    this.terminal_.setDrc(true, drcpointer);
    this.print(string + '\r\n');
};
...
hterm.TextAttributes = function(document) {
    ...
    this.underlineSource = this.SRC_DEFAULT; // These properties cache the value in the color table, but foregroundSource
    ...
    this.defaultBackground = 'rgb(0, 0, 0)'; // Any attributes added here that do not default to falsey (e.g. undefined or
...
hterm.TextAttributes.prototype.resetColor = function(index) {
    ...
    if (isNaN(index) || index >= this.colorPalette.length) return;
...
hterm.TextAttributes.prototype.isDefault = function() {
    return this.foregroundSource == this.SRC_DEFAULT && this.backgroundSource == this.SRC_DEFAULT && !this.bold && !this.faint && !this.italic && !this.blink && !this.underline && !this.strikethrough && !this.inverse && !this.invisible && !this.wcNode && this.asciiNode && this.tileData == null && this.uri == null;
};
/**
...
hterm.TextAttributes.prototype.createContainer = function(opt_textContent) {
    ...
    if (this.underlineColor != this.DEFAULT_COLOR) style.textDecorationColor = this.underlineColor;
...
hterm.TextAttributes.prototype.matchesContainer = function(obj) {
    if (typeof obj == 'string' || obj.nodeType == Node.TEXT_NODE) return this.isDefault();
    var style = obj.style; // We don't want to put multiple characters in a wcNode or a tile.
...
hterm.TextAttributes.prototype.syncColors = function() {
    ...
    var getDefaultColor = function getDefaultColor(color, defaultColor) {
        return color == _this17.DEFAULT_COLOR ? defaultColor : color;
    };
    ...
    if (foregroundSource == this.SRC_DEFAULT) this.foreground = this.DEFAULT_COLOR;
    else if (Number.isInteger(foregroundSource)) this.foreground = this.colorPalette[foregroundSource];
    else this.foreground = foregroundSource;
    if (this.faint) {
    ...
    if (backgroundSource == this.SRC_DEFAULT) this.background = this.DEFAULT_COLOR;
    else if (Number.isInteger(backgroundSource)) this.background = this.colorPalette[backgroundSource];
    else this.background = backgroundSource; // Once we've processed the bold-as-bright and faint attributes, swap.
    ...
    if (this.invisible) this.foreground = this.background;
    if (this.underlineSource == this.SRC_DEFAULT) this.underlineColor = this.DEFAULT_COLOR;
    else if (Number.isInteger(this.underlineSource)) this.underlineColor = this.colorPalette[this.underlineSource];
    else this.underlineColor = this.underlineSource;
};
/**
...
hterm.TextAttributes.containersMatch = function(obj1, obj2) {
    ...
    if (obj1.nodeType == Node.TEXT_NODE) return true;
...
hterm.VT = function(terminal) {
    ...
    this.mouseCoordinates = this.MOUSE_COORDINATES_X10; // We only want to report mouse moves between cells, not between pixels.
    ...
    this.codingSystemLocked_ = false; // Construct a regular expression to match the known one-byte control chars.
...
hterm.VT.ParseState = function(defaultFunction, opt_buf) {
    ...
    this.args = []; // Whether any of the arguments in the args array have subarguments.
...
hterm.VT.ParseState.prototype.argSetSubargs = function(argnum) {
    if (this.subargs === null) this.subargs = {};
...
hterm.VT.prototype.onTerminalMouse_ = function(e) { // Short circuit a few events to avoid unnecessary processing.
    ...
    else if (this.mouseReport != this.MOUSE_REPORT_DRAG && e.type == 'mousemove') return; // Temporary storage for our response.
    ...
    var x;
    var y; // Normally X10 has a limit of 255, but since we only want to emit UTF-8 valid
    ...
    switch (this.mouseCoordinates) {
        case this.MOUSE_COORDINATES_UTF8: // UTF-8 mode is the same as X10 but with higher limits.
            limit = 2047;
        case this.MOUSE_COORDINATES_X10: // X10 reports coordinates by encoding into strings.
            ...
            break;
        case this.MOUSE_COORDINATES_SGR: // SGR reports coordinates by transmitting the numbers directly.
            x = e.terminalColumn;
            y = e.terminalRow;
            break;
    }
    switch (e.type) {
        case 'wheel': // Mouse wheel is treated as button 1 or 2 plus an additional 64.
            ...
            if (this.mouseCoordinates == this.MOUSE_COORDINATES_SGR) {
                response = "\x1B[<".concat(b, ";").concat(x, ";").concat(y, "M");
            } else { // X10 based modes (including UTF8) add 32 for legacy encoding reasons.
                response = '\x1b[M' + String.fromCharCode(b + 32) + x + y;
            } // Keep the terminal from scrolling.
            e.preventDefault();
            break;
        case 'mousedown': // Buttons are encoded as button number.
            ...
            b |= mod;
            if (this.mouseCoordinates == this.MOUSE_COORDINATES_SGR) response = "\x1B[<".concat(b, ";").concat(x, ";").concat(y, "M");
            else response = '\x1b[M' + String.fromCharCode(b) + x + y;
            break;
        case 'mouseup':
            if (this.mouseReport != this.MOUSE_REPORT_PRESS) {
                if (this.mouseCoordinates == this.MOUSE_COORDINATES_SGR) { // SGR mode can report the released button.
                    response = "\x1B[<".concat(e.button, ";").concat(x, ";").concat(y, "m");
            ...
            break;
        case 'mousemove':
            if (this.mouseReport == this.MOUSE_REPORT_DRAG && e.buttons) { // Standard button bits.  The XTerm protocol only reports the first
                ...
                b |= mod;
                if (this.mouseCoordinates == this.MOUSE_COORDINATES_SGR) response = "\x1B[<".concat(b, ";").concat(x, ";").concat(y, "M");
                else response = '\x1b[M' + String.fromCharCode(b) + x + y; // If we were going to report the same cell because we moved pixels
                // within, suppress the report.  This is what xterm does and cuts
                // down on duplicate messages.
                if (this.lastMouseDragResponse_ == response) response = '';
                else this.lastMouseDragResponse_ = response;
            }
            break;
        case 'click':
        case 'dblclick':
...
hterm.VT.prototype.setEncoding = function(encoding) {
    switch (encoding) {
        default:
            console.warn('Invalid value for "terminal-encoding": ' + encoding); // Fall through.
        case 'iso-2022':
            this.codingSystemUtf8_ = false;
...
hterm.VT.prototype.updateEncodingState_ = function() {
    var _this18 = this; // If we're in UTF8 mode, don't suport 8-bit escape sequences as we'll never
    // see those -- everything should be UTF8!
    var cc1 = Object.keys(hterm.VT.CC1).filter(function(e) {
        return !_this18.codingSystemUtf8_ || e.charCodeAt() < 0x80;
    }).map(function(e) {
        return '\\x' + lib.f.zpad(e.charCodeAt().toString(16), 2);
    }).join('');
    this.cc1Pattern_ = new RegExp("[".concat(cc1, "]"));
};
/**
...
hterm.VT.prototype.parseUnknown_ = function(parseState) {
    ...
    function print(str) {
        if (!self.codingSystemUtf8_ && self[self.GL].GL) str = self[self.GL].GL(str);
    ...
    if (nextControl == 0) { // We've stumbled right into a control character.
...
hterm.VT.prototype.parseCSI_ = function(parseState) {
    ...
    var finishParsing = function finishParsing() { // Resetting the arguments isn't strictly necessary, but it makes debugging
...
hterm.VT.prototype.parseUntilStringTerminator_ = function(parseState) {
    var buf = parseState.peekRemainingBuf();
    var args = parseState.args; // Since we might modify parse state buffer locally, if we want to advance
    // the parse state buffer later on, we need to know how many chars we added.
    var bufInserted = 0;
    if (!args.length) {
        args[0] = '';
        args[1] = new Date();
    } else { // If our saved buffer ends with an escape, it's because we were hoping
    ...
    var terminator = buf[nextTerminator];
    var foundTerminator; // If the next escape we see is not a start of a ST, fall through.  This will
    // either be invalid (embedded escape), or we'll queue it up (wait for \\).
    if (terminator == '\x1b' && buf[nextTerminator + 1] != '\\') foundTerminator = false;
    else foundTerminator = nextTerminator != -1;
    if (!foundTerminator) { // No terminator here, have to wait for the next string.
        args[0] += buf;
        var abortReason; // Special case: If our buffering happens to split the ST (\e\\), we have to
        ...
        if (terminator == '\x1b' && nextTerminator != buf.length - 1) abortReason = 'embedded escape: ' + nextTerminator;
        if (new Date() - args[1] > this.oscTimeLimit_) abortReason = 'timeout expired: ' + (new Date() - args[1]);
        if (abortReason) {
            if (this.warnUnimplemented) console.log('parseUntilStringTerminator_: aborting: ' + abortReason, args[0]);
    ...
    parseState.resetParseFunction();
    parseState.advance(nextTerminator + (terminator == '\x1b' ? 2 : 1) - bufInserted);
    return true;
};
/**
...
hterm.VT.prototype.dispatch = function(type, code, parseState) {
    ...
    if (parseState.subargs && !handler.supportsSubargs) {
        if (this.warnUnimplemented) console.warn('Ignored ' + type + ' code w/subargs: ' + JSON.stringify(code));
        return;
    }
    if (type == 'CC1' && code > '\x7f' && !this.enable8BitControl) { // It's kind of a hack to put this here, but...
        //
...
hterm.VT.prototype.setDECMode = function(code, state) {
    switch (parseInt(code, 10)) {
        ...
        case 1048: // Save cursor as in DECSC.
            if (state) this.terminal.saveCursorAndState();
            else this.terminal.restoreCursorAndState();
            break;
    ...
    parseState.func = function(parseState) {
        ...
        switch (ch) {
            ...
            case 'G': // Switch to UTF-8.
                ...
                break;
            case '/': // One way transition to something else.
                ...
                switch (ch) {
                    ...
                    case 'I': // UTF-8 Level 3.
                        ...
                        break;
                    default:
                        if (this.warnUnimplemented) console.warn('Unknown ESC % / argument: ' + JSON.stringify(ch));
                        break;
                }
                break;
            default:
...
hterm.VT.OSC['4'] = function(parseState) { // Args come in as a single 'index1;rgb1 ... ;indexN;rgbN' string.
    ...
    for (var pairNumber = 0; pairNumber < pairCount; ++pairNumber) {
        ...
        if (colorValue == '?') { // '?' means we should report back the current color value.
...
hterm.VT.OSC['8'] = function(parseState) {
    ...
    var id = null;
    var uri = null;
    if (args.length != 2 || args[1].length == 0) { // Reset link.
...
hterm.VT.OSC['12'] = function(parseState) { // Args come in as a single string, but extra args will chain to the following
    ...
    var colorArg;
    var colorX11 = lib.colors.x11ToCSS(args.shift());
    if (colorX11) this.terminal.setCursorColor(colorX11);
...
hterm.VT.OSC['52'] = function(parseState) {
    ...
    if (this.characterEncoding == 'utf-8') {
        ...
        var bytes = lib.codec.stringToCodeUnitArray(data, Uint8Array);
        data = decoder.decode(bytes);
    }
    if (data) this.terminal.copyStringToClipboard(data);
};
/**
 * Reset color palette.
 */
...
hterm.VT.OSC['1337'] = function(parseState) { // Args come in as a set of key value pairs followed by data.
    ...
    if (!args) {
        if (this.warnUnimplemented) console.log("iTerm2 1337: unsupported sequence: ".concat(args[1]));
        return;
    }
    var options = {
        name: '',
    ...
    args[1].split(';').forEach(function(ele) {
        var kv = ele.match(/^([^=]+)=(.*)$/m);
        if (!kv) return; // Sanitize values nicely.
        switch (kv[1]) {
            case 'name':
                try {
            ...
            case 'type':
                options.type = kv[2];
                break;
            default: // Ignore unknown keys.  Don't want remote stuffing our JS env.
                break;
        }
    ...
    if (options.inline) {
        var io = this.terminal.io;
        var queued = parseState.peekRemainingBuf();
...
hterm.VT.prototype.parseSgrExtendedColors = function(parseState, i, attrs) {
    var ary;
    var usedSubargs;
    if (parseState.argHasSubargs(i)) { // The ISO 8613-6 compliant form.
    ...
    switch (parseInt(ary[0])) {
        ...
        case 2: { // RGB color.
            ...
            var start;
            if (usedSubargs) { // The ISO 8613-6 compliant form:
            ...
            if (ary.length < start + 3) return {
                skipCount: 0
            };
            var r = parseState.parseInt(ary[start + 0]);
            var g = parseState.parseInt(ary[start + 1]);
            var b = parseState.parseInt(ary[start + 2]);
            return {
        ...
        case 3: { // CMY color.
            ...
            // tolerance, etc...
            if (ary.length < 4) return {
                skipCount: 0
            }; // TODO: See CMYK below.
            var c = parseState.parseInt(ary[1]);
            var m = parseState.parseInt(ary[2]);
            var y = parseState.parseInt(ary[3]);
            return {
                skipCount: 0
            };
        }
        case 4: { // CMYK color.
            ...
            var _c = parseState.parseInt(ary[1]);
            var _m = parseState.parseInt(ary[2]);
            var _y = parseState.parseInt(ary[3]);
            var k = parseState.parseInt(ary[4]);
            return {
                skipCount: 0
            };
        }
        case 5: { // Color palette index.
            ...
            var color = parseState.parseInt(ary[1]);
...
hterm.VT.CSI['m'] = function(parseState) {
    ...
    for (var i = 0; i < parseState.args.length; i++) { // If this argument has subargs (i.e. it has args followed by colons),
        ...
        if (arg < 30) {
            if (arg == 0) { // Normal (default).
                ...
                if (parseState.argHasSubargs(i)) {
                    ...
                    if (uarg == 0) attrs.underline = false;
                    else if (uarg == 1) attrs.underline = 'solid';
                    else if (uarg == 2) attrs.underline = 'double';
                    else if (uarg == 3) attrs.underline = 'wavy';
                    else if (uarg == 4) attrs.underline = 'dotted';
                    else if (uarg == 5) attrs.underline = 'dashed';
                } else {
            ...
            if (arg < 38) {
                ...
                if (result.color !== undefined) attrs.foregroundSource = result.color;
                i += result.skipCount;
            } else if (arg == 39) {
                ...
                var _result = this.parseSgrExtendedColors(parseState, i, attrs);
                if (_result.color !== undefined) attrs.backgroundSource = _result.color;
                i += _result.skipCount;
            } else {
                attrs.backgroundSource = attrs.SRC_DEFAULT;
            }
        } else if (arg == 58) { // Underline coloring.
            var _result2 = this.parseSgrExtendedColors(parseState, i, attrs);
            if (_result2.color !== undefined) attrs.underlineSource = _result2.color;
            i += _result2.skipCount;
...
hterm.VT.CSI['z'] = function(parseState) {
    ...
    var arg = parseState.args[0];
    if (arg == 0) { // Start a glyph (one parameter, the glyph number).
...
hterm.VT.CharacterMap.prototype.sync_ = function(opt_glmap) {
    var _this19 = this; // If there are no maps, then reset the state back.
    ...
    if (opt_glmap) this.glmap_ = Object.assign({}, this.glmapBase_, opt_glmap);
    else this.glmap_ = this.glmapBase_;
    ...
    this.GL = function(str) {
        return str.replace(_this19.glre_, function(ch) {
            return _this19.glmap_[ch];
        });
    };
};
/**
...
hterm.VT.CharacterMap.prototype.clone = function() {
    ...
    if (this.glmap_ !== this.glmapBase_) map.setOverrides(this.glmap_);
    return map;
};
/**
...
hterm.VT.CharacterMaps.prototype.getMap = function(name) {
    if (this.maps_.hasOwnProperty(name)) return this.maps_[name];
...
hterm.VT.CharacterMaps.prototype.addMap = function(name, map) {
    if (this.maps_ === this.mapsBase_) this.maps_ = Object.assign({}, this.mapsBase_);
    this.maps_[name] = map;
};
/**
...
hterm.VT.CharacterMaps.prototype.reset = function() {
    if (this.maps_ !== hterm.VT.CharacterMaps.DefaultMaps) this.maps_ = hterm.VT.CharacterMaps.DefaultMaps;
};
/**
...
hterm.VT.CharacterMaps.prototype.setOverrides = function(maps) {
    ...
    for (var name in maps) {
        ...
        } else this.addMap(name, new hterm.VT.CharacterMap('user ' + name, maps[name]));
    }
};
/**
...
lib.resource.add('hterm/changelog/version', 'text/plain', '1.84');
lib.resource.add('hterm/changelog/date', 'text/plain', '2019-01-19');
lib.resource.add('hterm/git/HEAD', 'text/plain', '251d68966c9b2e32e2c127f5ce99653849e58520');