...
public class CorrectLabel {
	...
	public static String nameErrors(String label, String errorIdentifierString) {
		...
		for (var i = 0; i < label.length(); i++) {
			if (!CHARS.contains(label.toLowerCase().substring(i, i + 1))
				...
				return errorIdentifierString + LC.getFormatted("IllegalChar", label.substring(i, i + 1));
			}
		}
		if (VERILOG_KEYWORDS.contains(label)) {
			return errorIdentifierString + LC.get("ReservedVerilogKeyword");
		}
		return null;
	}
	...
	public static boolean isKeyword(String label, Boolean showDialog) {
		...
		if (VERILOG_KEYWORDS.contains(label.toLowerCase())) {
			if (showDialog) DialogManager.createErrorDialog("Error!", LC.get("VerilogKeywordNameError"));
			ret = true;
		}
		return ret;
	}
