...
public class Netlist {
	...
	public int designRuleCheckResult(boolean isTopLevel, ArrayList<String> sheetNames) {
		...
		if (myCircuit.getName().isEmpty()) {
			...
			Reporter.report.addFatalError(LC.get("EmptyNamedSheet"));
			drcStatus |= DRC_ERROR;
		}
		if (sheetNames.contains(myCircuit.getName())) {
			// in the current implementation of logisim this should never
			// happen, but we leave it in
			Reporter.report.addFatalError(LC.getFormatted("MultipleSheetSameName", myCircuit.getName()));
			drcStatus |= DRC_ERROR;
		} else {
			...
			if (comp.getFactory().requiresNonZeroLabel()) {
				...
				if (comp.getFactory() instanceof SubcircuitFactory){
					...
					if (!CorrectLabel.isCorrectLabel(
							...
							LC.getFormatted("FoundBadComponent", comp.getFactory().getName(), myCircuit.getName()))) {
						drcStatus |= DRC_ERROR;
					}
		 ...
		 * the net list
		 */
		Reporter.report.addInfo(LC.getFormatted("BuildingNetlistFor", myCircuit.getName()));
		...
		if (isTopLevel) {
			...
			if (ports == 0) {
				Reporter.report.addFatalError(LC.getFormatted("TopLevelNoIO", myCircuit.getName()));
				drcStatus = DRC_ERROR;
				return drcStatus;
			}
		...
		Reporter.report.addInfo(LC.getFormatted("CircuitInfoString", myCircuit.getName(), Integer.toString(numberOfNets()), Integer.toString(numberOfBusses())));
		Reporter.report.addInfo(LC.getFormatted("DRCPassesString", myCircuit.getName()));
		drcStatus = DRC_PASSED;
		return drcStatus;
	}
	...
	private boolean detectGatedClocks() {
		...
		for (final var key : notGatedSet.keySet()) {
			if (gatedSet.containsKey(key)) {
				/* big Problem, we have a component that is used with and without gated clocks */
				Reporter.report.addSevereWarning(LC.get("NetList_CircuitGatedNotGated"));
	...
	public void getGatedClockComponents(
		...
		if (hierarchyNetlists.size() > 1) {
			...
			warningForGatedClock(nonPinSources, nonPinGatedComponents, nonPinWires, warnedComponents, hierarchyNetlists, LC.get("NetList_GatedClock"));
			warningForGatedClock(pinSources, pinGatedComponents, pinWires, warnedComponents, hierarchyNetlists, LC.get("NetList_PossibleGatedClock"));
		}
	}
	...
	public ArrayList<netlistComponent> getInputPorts(){
		return myInputPorts;
	}

	public ArrayList<netlistComponent> getOutputPorts(){
		return myOutputPorts;
	}

	public ArrayList<netlistComponent> getInOutPorts(){
		return myInOutPorts;
	}

	public ArrayList<netlistComponent> getClockGenerators(){
		return myClockGenerators;
	}

	public ArrayList<netlistComponent> getComponents(){
		return myComponents;
	}
