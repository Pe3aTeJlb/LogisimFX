...
import LogisimFX.std.io.SevenSegment;
...
public class MapComponent {
	...
	public void tryMap(CircuitMapInfo cmap, List<FpgaIoInformationContainer> IOcomps) {
		...
		if (cmap.getPinMaps() == null) {
			...
			for (var comp : IOcomps) {
				if (comp.getRectangle().contains(rect.getX(), rect.getY())) {
					if (cmap.isSinglePin()) {
				...
				if (isPinMapped) {
					...
					if (rect1.equals(rect2) && rect2.equals(rect3)) {
						...
						if (iomap1 == iomap2 && iomap2 == iomap3) {
							...
							for (var comp : IOcomps) {
								if (comp.getRectangle().contains(rect1.getX(), rect1.getY())) {
			...
			try {
				...
				for (var comp : IOcomps) {
					if (comp.getRectangle()
							.contains(cmap.getRectangle().getX(), cmap.getRectangle().getY())) {
	...
	public String getDisplayString(int pin) {
		...
		if (pin >= 0) {
			...
			if (opens.get(pin)) s.append("->").append(LC.get("MapOpen"));
	...
	public void getMapElement(Element Map) throws DOMException {
		...
		if (isCompleteMap(true)) {
			if (opens.get(0)) {
				...
				Map.setAttribute(COMPLETE_MAP, (int)rect.getX() + "," +(int)rect.getY());
			}
		} else {
			var s = new StringBuilder();
			var first = true;
			for (var i = 0; i < nrOfPins; i++) {
				...
				if (opens.get(i)) s.append(OPEN_KEY);
					...
					var map = maps.get(i);
					s.append((int)map.IOcomp.getRectangle().getX())
							.append("_")
							.append((int)map.IOcomp.getRectangle().getY())
							.append("_")
							.append(map.pin);
	...
	public static void getComplexMap(Element Map, CircuitMapInfo cmap) throws DOMException {
		...
		if (pinmaps != null) {
			...
			for (CircuitMapInfo pinmap : pinmaps) {
				...
				if (pinmap == null) {
					...
					if (pinmap.isConst()) s.append(pinmap.getConstValue());
						...
						s.append((int)pinmap.getRectangle().getX())
								.append("_")
								.append((int)pinmap.getRectangle().getY())
								.append("_")
			...
			if (br == null) return;
			Map.setAttribute(COMPLETE_MAP, (int)br.getX() + "," + (int)br.getY());
		}
	}
