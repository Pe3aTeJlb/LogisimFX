...
public enum IoComponentTypes {
	...
	public static int getFpgaInOutRequirement(IoComponentTypes comp) {
		switch (comp) {
			case PortIo: return 8;
			case LocalBus: return 16;
			default: return 0;
		}
	}
	...
	public static int getFpgaInputRequirement(IoComponentTypes comp) {
		switch (comp) {
			...
			case DIPSwitch: return 8;
			case LocalBus: return 13;
			default: return 0;
		}
	}
	...
	public static int getFpgaOutputRequirement(IoComponentTypes comp) {
		switch (comp) {
			case Led: return 1;
			case SevenSegment: return 8;
			case SevenSegmentNoDp: return 7;
			case RgbLed: return 3;
			case LocalBus: return 2;
			case LedArray: return 16;
			default: return 0;
		}
	}
	...
	public static String getInputLabel(int nrPins, int id, IoComponentTypes comp) {
		if (comp == IoComponentTypes.DIPSwitch) {
			return DipSwitch.getInputLabel(id);
		}
		return (nrPins > 1) ? LC.getFormatted("FpgaIoPins", Integer.toString(id)) : LC.get("FpgaIoPin");
	}
	...
	public static String getRotationString(IoComponentTypes comp, int rotation) {
		switch (comp) {
			case DIPSwitch: switch (rotation) {
				case ROTATION_CW_90: return LC.get("DipSwitchCW90");
				case ROTATION_CCW_90: return LC.get("DipSwitchCCW90");
				default: return LC.get("DipSwitchZero");
			}
			case SevenSegment: switch (rotation) {
				case ROTATION_CW_90: return LC.get("SevenSegmentCW90");
				case ROTATION_CCW_90: return LC.get("SevenSegmentCCW90");
				default: return LC.get("SevenSegmentZero");
			}
			case LedArray: switch (rotation) {
				case ROTATION_CW_90: return LC.get("LEDArrayCW90");
				case ROTATION_CCW_90: return LC.get("LEDArrayCCW90");
				default: return LC.get("LEDArrayZero");
			}
	...
	public static String getOutputLabel(int nrPins, int nrOfRows, int nrOfColumns, int id, IoComponentTypes comp) {
		switch (comp) {
			...
			case SevenSegment:
				return LogisimFX.std.io.SevenSegment.getOutputLabel(id);
			case RgbLed:
				return LogisimFX.std.io.RgbLed.getLabel(id);
			case LedArray:
	...
	private static int[][] getSevenSegmentDisplayArray(boolean hasDp) {
		final var sa = LogisimFX.std.io.SevenSegment.Segment_A;
		final var sb = LogisimFX.std.io.SevenSegment.Segment_B;
		final var sc = LogisimFX.std.io.SevenSegment.Segment_C;
		final var sd = LogisimFX.std.io.SevenSegment.Segment_D;
		final var se = LogisimFX.std.io.SevenSegment.Segment_E;
		final var sf = LogisimFX.std.io.SevenSegment.Segment_F;
		final var sg = LogisimFX.std.io.SevenSegment.Segment_G;
		final int[][] indexes = {
				{-1, sa, sa, -1, -1},
				{sf, -1, -1, sb, -1},
				{sf, -1, -1, sb, -1},
		...
		if (hasDp) indexes[6][4] = LogisimFX.std.io.SevenSegment.DP;
		return indexes;
	}
		...
		switch (type) {
			case DIPSwitch:
				switch (mapRotation) {
					...
					case ROTATION_CW_90: part = (float) height / (float) nrOfPins; break;
					default: part = (float) width / (float) nrOfPins; break;
				};
				for (var widthIndex = 0; widthIndex < width; widthIndex++)
					for (var heightIndex = 0; heightIndex < height; heightIndex++) {
						var pinIndex = 0;
						switch (mapRotation) {
							case ROTATION_CCW_90: pinIndex = (int) ((height - heightIndex - 1) / part); break;
							case ROTATION_CW_90: pinIndex = (int) (height / part); break;
							default: pinIndex = (int) (widthIndex / part); break;
						};
