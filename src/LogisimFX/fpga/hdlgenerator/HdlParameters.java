...
import LogisimFX.std.gates.GateAttributes;
import LogisimFX.std.gates.NegateAttribute;
	...
	private static class ParameterInfo {
				...
				case MAP_MULTIPLY:
					if (args.length == 2) {
						for (var arg = 0; arg < 2; arg++) {
							if (!(args[arg] instanceof Attribute<?>))
		...
		public String getParameterValue(AttributeSet attrs) {
			...
			switch (myMapType) {
				...
				case MAP_ATTRIBUTE_OPTION:
					if (!attrs.containsAttribute(attributesList.get(0)))
						throw new UnsupportedOperationException(noReqAttrExMsg);
					...
					if (!attributeOptionMap.containsKey(value))
					...
					if (attributesList.isEmpty()) {
						...
						for (final var attr : attributesList) {
							if (attrs.containsAttribute(attr)) {
								...
								if (attrValue instanceof Integer) selectedValue *= (Integer) attrValue;
								else if (attrValue instanceof Long) selectedValue *= (Long) attrValue;
								else if (attrValue instanceof BitWidth) selectedValue *= ((BitWidth)attrValue).getWidth();
								else
		...
		private long getCorrectIntValue(Object... args) {
			...
			var value = 0L;
			if (args[0] instanceof Integer) value = (Integer) args[0];
