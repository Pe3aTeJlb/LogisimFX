...
public class CircuitHdlGeneratorFactory extends AbstractHdlGeneratorFactory {
	...
	private String getBubbleIndex(netlistComponent comp, bubbleType type) {
		final var fmt = "{{<}}{{1}} {{2}} {{3}}{{>}}";
		switch (type) {
			case INPUT: return LineBuffer.format(fmt, comp.getLocalBubbleInputEndId(),
					Hdl.vectorLoopId(), comp.getLocalBubbleInputStartId());
			case OUTPUT: return LineBuffer.format(fmt, comp.getLocalBubbleOutputEndId(),
					Hdl.vectorLoopId(), comp.getLocalBubbleOutputStartId());
			case INOUT: return LineBuffer.format(fmt, comp.getLocalBubbleInOutEndId(),
		...
		for (final var comp : theNetList.getNormalComponents()) {
			...
			if (worker != null) {
				if (worker.isOnlyInlined()) {
					...
					final var remarkLine = LineBuffer.format("{{1}}{{2}}{{3}}", comp.getComponent().getFactory().getDisplayName().getValue(),
							hasLabel ? ": " : "", compName);
					LineBuffer inlined = worker.getInlinedCode(theNetList, id++, comp, inlinedName);
					if (!inlined.isEmpty()) {
						contents
								.empty()
