...
public class Parser {
			...
			if (token.type == TOKEN_ERROR) {
				...
				if (index < 0) {
					...
					if (opText.equals("NOT")) {
						...
						throw token.error(LC.createComplexStringBinding("badVariableName", token.text));
					}
				}
			}
		}
	...
	*/
	...
	private static class Token {
		...
		ParserException error(StringBinding message) {
	...
	private static Expression parse(ArrayList<Token> tokens) throws ParserException {
			...
			if (t.type == TOKEN_IDENT || t.type == TOKEN_CONST) {
				...
				if (current != null) {
					push(stack, current, Expression.AND_LEVEL,
						new Token(TOKEN_AND, t.offset, LC.get("implicitAndOperator")));
				}
				push(stack, null, Expression.NOT_LEVEL, t);
				current = null;
			} else if (t.type == TOKEN_NOT_POSTFIX) {
				throw t.error(LC.createStringBinding("unexpectedApostrophe"));
			} else if (t.type == TOKEN_LPAREN) {
				if (current != null) {
					push(stack, current, Expression.AND_LEVEL,
							new Token(TOKEN_AND, t.offset, 0, LC.get("implicitAndOperator")));
				}
				push(stack, null, -2, t);
				current = null;
				...
				if (stack.isEmpty()) {
					throw t.error(LC.createStringBinding("lparenMissingError"));
				}
				pop(stack);
				while (i + 1 < tokens.size() && tokens.get(i + 1).type == TOKEN_NOT_POSTFIX) {
				...
				if (current == null) {
					throw t.error(LC.createComplexStringBinding("missingLeftOperandError", t.text));
				}
				int level = 0;
				switch (t.type) {
		...
		if (!stack.isEmpty()) {
			Context top = pop(stack);
			throw top.cause.error(LC.createStringBinding("rparenMissingError"));
		}
		return current;
	}
	...
	private static Expression popTo(ArrayList<Context> stack, int level,
		...
		while (!stack.isEmpty() && peekLevel(stack) >= level) {
			Context top = pop(stack);
			if (current == null) throw top.cause.error(LC.createComplexStringBinding("missingRightOperandError", top.cause.text));
			switch (top.level) {
