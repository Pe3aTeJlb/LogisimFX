...
//        openProjectConstrains.disableProperty().bind(proj.getFpgaToolchainOrchestrator().constrainsFileProperty().isNull());
		...
		MenuItem openFPGAPiplineCommander = new MenuItem();
//        openProjectConstrains.disableProperty().bind(proj.getFpgaToolchainOrchestrator().constrainsFileProperty().isNull());
		openFPGAPiplineCommander.textProperty().bind(localizer.createStringBinding("fpgaOpenFPGAToolchainCommander"));
		openFPGAPiplineCommander.setOnAction(event -> FrameManager.CreateFPGACommanderCommanderFrame(proj));

		SeparatorMenuItem sp3 = new SeparatorMenuItem();

		MenuItem exportFiles = new MenuItem();
		exportFiles.textProperty().bind(localizer.createStringBinding("fpgaExportFilesItem"));
		exportFiles.setOnAction(event -> proj.getFpgaToolchainOrchestrator().execute(0));

		MenuItem generateBit = new MenuItem();
		generateBit.textProperty().bind(localizer.createStringBinding("fpgaGenerateBitFile"));
		generateBit.setOnAction(event -> proj.getFpgaToolchainOrchestrator().execute(1));
