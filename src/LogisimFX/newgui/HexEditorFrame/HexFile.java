...
public class HexFile {
		...
		public int[] next() throws IOException {
			...
			for (String tok = nextToken(); tok != null; tok = nextToken()) {
				try {
					...
					throw new IOException(LC.get("hexNumberFormatError"));
				}
	...
	public static void save(Writer out, HexModel src) throws IOException {
		...
		while (cur <= last) {
			...
			try {
				...
				throw new IOException(LC.get("hexFileWriteError"));
			}
			tokens++;
		}
	...
	public static void open(HexModel dst, Reader in) throws IOException {
		...
		while (reader.hasNext()) {
			...
			if (offs + values.length - 1 > dst.getLastOffset()) {
				throw new IOException(LC.get("hexFileSizeError"));
			}
			dst.set(offs, values);
			offs += values.length;
		}
	...
	public static void open(HexModel dst, File src) throws IOException {
		...
		try {
			...
			throw new IOException(LC.get("hexFileOpenError"));
		}
		try {
			String header = in.readLine();
			if (!header.equals(RAW_IMAGE_HEADER)) {
				throw new IOException(LC.get("hexHeaderFormatError"));
			}
			open(dst, in);
			try {
				...
				throw new IOException(LC.get("hexFileReadError"));
			}
		} finally {
			try {
				if (in != null) in.close();
