...
public class RgbLed extends InstanceFactory {
	...
	public RgbLed() {
		super("RGBLED", LC.createStringBinding("RGBledComponent"), new AbstractSimpleIoHdlGeneratorFactory(false), true);
		setAttributes(
				new Attribute[]{
	...
	private void updatePorts(Instance instance) {
		...
		ps[RED].setToolTip(LC.createStringBinding("RED"));
		ps[GREEN].setToolTip(LC.createStringBinding("GREEN"));
		ps[BLUE].setToolTip(LC.createStringBinding("BLUE"));
	...
	public void paintGhost(InstancePainter painter) {
		...
		painter.getGraphics().setLineWidth(2);
		painter.getGraphics().c.strokeOval(bds.getX() + 1, bds.getY() + 1, bds.getWidth() - 2, bds.getHeight() - 2);
		painter.getGraphics().toDefaultLineWidth();
	}
	...
	public void paintInstance(InstancePainter painter) {
		...
		if (painter.getShowState()) {
			final boolean activ = painter.getAttributeValue(Io.ATTR_ACTIVE);
			final int mask = activ ? 0 : 7;
			sum ^= mask;
			final var red = ((sum >> RED) & 1) * 0xFF;
			final var green = ((sum >> GREEN) & 1) * 0xFF;
			final var blue = ((sum >> BLUE) & 1) * 0xFF;
			final Color ledColor = new Color((double) red/255, (double) green/255, (double) blue/255, 1);
			g.setColor(ledColor);
