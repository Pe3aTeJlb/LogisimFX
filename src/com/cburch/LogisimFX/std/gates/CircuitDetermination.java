...
import com.cburch.LogisimFX.analyze.model.ExpressionVisitor;
	...
	static class Gate extends CircuitDetermination {
		...
		private CircuitDetermination convertToTwoInputsSub(int start, int stop,
			...
			if (stop - start == 1) {
				...
				CircuitDetermination.Gate ret = new CircuitDetermination.Gate(subFactory);
				ret.inputs.add(a);
				ret.inputs.add(b);
				return ret;
			}
		...
		private void notOutput() {
			CircuitDetermination.Gate sub = new CircuitDetermination.Gate(NandGate.FACTORY);
		...
		private void notAllInputs() {
			for (int i = 0; i < inputs.size(); i++) {
				...
				if (old.isNandNot()) {
					inputs.set(i, ((CircuitDetermination.Gate) old).inputs.get(0));
				} else {
					CircuitDetermination.Gate now = new CircuitDetermination.Gate(NandGate.FACTORY);
					now.inputs.add(old);
					now.inputs.add(old);
					inputs.set(i, now);
				}
		...
		void repair() {
			...
			if (num > GateAttributes.MAX_INPUTS) {
				...
				for (int i = 0; i < newNum; i++) {
					CircuitDetermination.Gate sub = new CircuitDetermination.Gate(subFactory);
