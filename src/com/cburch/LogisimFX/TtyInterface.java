...
import com.cburch.LogisimFX.std.memory.Ram;
...
public class TtyInterface {
	...
	public static void run(Startup args) {
		...
		if (args.getLoadFile() != null) {
			try {
				...
				if (!loaded) {
					System.err.println(lc.get("loadNoRamError")); //OK
					System.exit(-1);
				}
			} catch (IOException e) {
				System.err.println(lc.get("loadIoError") + ": " + e.toString()); //OK
				System.exit(-1);
			}
		}
	...
	private static int runSimulation(CircuitState circState,
		...
		if (showTty) {
			...
			if (!ttyFound) {
				System.err.println(lc.get("ttyNoTtyError")); //OK
				System.exit(-1);
			}
		...
		if (showHalt || retCode != 0) {
			if (retCode == 0) {
				System.out.println(lc.get("ttyHaltReasonPin")); //OK
			} else if (retCode == 1) {
				System.out.println(lc.get("ttyHaltReasonOscillation")); //OK
			}
		}
		if (showSpeed) {
	...
	private static void displaySpeed(long tickCount, long elapse) {
		...
		System.out.println(StringUtil.format(lc.get("ttySpeedMsg"), //OK
